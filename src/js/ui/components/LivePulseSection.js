...
import * as liveNewsService from '../../services/live-news.service.js?v=bd772441';
const { 
...
export function mountLivePulseSection(containerId) {
  ...
  // Start 15-Minute Automatic News & Jobs Sync Cycle
  try {
    startFifteenMinuteNewsSync(() => {
      renderNewsCards();
    });

    // Auto re-render every 15 minutes to refresh timeAgo labels and fetch new incoming updates
    setInterval(() => {
      renderNewsCards();
    }, 15 * 60 * 1000);
  } catch (_) {}
