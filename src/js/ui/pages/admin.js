...
async function renderAdminOverview($container) {
  ...
  try {
    ...
    const productsList = Array.isArray(adminCache.products) ? adminCache.products : [];
...
async function renderAdminSettings($container) {
  ...
  document.getElementById('admin-settings-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const fullSettings = {
        general: {
          siteName: document.getElementById('s-site-name')?.value || 'المنزلة وناسها',
          primaryColor: document.getElementById('s-color')?.value || '#1B4F72',
          siteDescription: document.getElementById('s-desc')?.value || ''
        },
        contact: {
          whatsappLink: document.getElementById('s-wa-link')?.value || '',
        ...
        telegram: {
          ...
          adminChatId: (document.getElementById('s-tg-chat-id')?.value || '').trim()
        },
        limits: {
          offersVerified: Number(document.getElementById('s-lim-off-ver')?.value) || 3,
          offersUnverified: Number(document.getElementById('s-lim-off-unver')?.value) || 1,
          productsVerified: Number(document.getElementById('s-lim-prod-ver')?.value) || 350
        }
      };

      await updateSettings(fullSettings);
      await dbUpdate('settings', updates).catch(() => {});
      adminCache.settings = fullSettings;
    ...
    content: `
      <form id="admin-edit-place-form" style="display:flex;flex-direction:column;gap:14px;max-height:72vh;overflow-y:auto;padding:4px 8px" onsubmit="return false;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px">
          ...
          <div class="form-group">
            <label class="form-label">نوع وسيلة التوصيل / النقل (إن وجد)</label>
            <select id="aep-deliveryType" class="form-select">
              <option value="" ${!place.deliveryType ? 'selected' : ''}>بدون (نشاط عادي)</option>
              <option value="motorcycle" ${place.deliveryType === 'motorcycle' ? 'selected' : ''}>🏍️ موتوسيكل دليفري</option>
              <option value="tuktuk" ${place.deliveryType === 'tuktuk' ? 'selected' : ''}>🛺 توكتوك مشاوير</option>
              <option value="car" ${place.deliveryType === 'car' ? 'selected' : ''}>🚗 سيارة ملاكي / رحلات</option>
    ...
    buttons: [
      {
        ...
        onClick: async () => {
          ...
          try {
            ...
            toast.success('تم تحديث التصنيف وحفظه بنجاح');
            modal.close();
            switchAdminSection('categories', false);
          } catch {
