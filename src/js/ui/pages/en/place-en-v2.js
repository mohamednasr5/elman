...
import { formatWorkingHours, isPlaceOpen } from '../../../utils/date.js';
import { resolveMapEmbedInfo } from '../../../utils/maps.js';
...
const englishOnly = (value, fallback = '') => englishText(value, fallback);
const hasSocial = social => social && typeof social === 'object' && Object.values(social).some(Boolean);
const stars = rating => { const n=Math.min(5,Math.max(0,Math.round(Number(rating)||0))); return '★'.repeat(n)+'☆'.repeat(5-n); };
const isVerified = place => Boolean(place?.isVerified || place?.is_verified || place?.verified);

function verifiedBadge() {
  return '<span class="en-verified-badge" title="Verified business" aria-label="Verified business"><span class="en-verified-badge__icon">✓</span><span>Verified</span></span>';
}

function availabilityBadge(status) {
  const s=String(status||'').toLowerCase();
  if (!s) return '';
  const map={available:['Available now','is-open'],busy:['Busy right now','is-busy'],unavailable:['Unavailable','is-closed']};
  const item=map[s]; if(!item) return '';
  return `<span class="en-status-pill ${item[1]}"><span aria-hidden="true">●</span>${item[0]}</span>`;
}

function socialLinks(social = {}) {
  const items=[['facebook','Facebook'],['instagram','Instagram'],['tiktok','TikTok'],['youtube','YouTube'],['threads','Threads'],['x','X'],['twitter','X'],['website','Website']];
  return items.filter(([key])=>social[key]).filter((item,index,arr)=>item[1]!=='X'||index===arr.findIndex(x=>x[1]==='X')).map(([key,label])=>`<a class="en-social-link" href="${esc(social[key])}" target="_blank" rel="noopener noreferrer">${label}</a>`).join('');
}

function renderHours(hours) {
  const list=Array.isArray(hours)?hours:[];
  if(!list.length) return '<div class="en-empty-inline">Business hours are not available yet.</div>';
  const names={saturday:'Saturday',sunday:'Sunday',monday:'Monday',tuesday:'Tuesday',wednesday:'Wednesday',thursday:'Thursday',friday:'Friday'};
  return list.map(h=>`<div class="en-hours-row ${h.isToday?'is-today':''}"><span class="en-hours-day">${esc(names[h.key]||englishOnly(h.name,'Day'))}${h.isToday?' <em>Today</em>':''}</span><span class="en-hours-time ${h.closed?'is-closed':''}">${h.closed?'Closed':`${esc(h.open)} — ${esc(h.close)}`}</span></div>`).join('');
}

function renderOffer(offer) {
  const title=englishOnly(offer.title,'Special Offer'); const description=englishOnly(offer.description,'');
  const image=offer.imageUrl?getOptimizedImageUrl(offer.imageUrl,IMAGE_SIZES.CARD):''; const price=englishOnly(offer.newPrice,''); const oldPrice=englishOnly(offer.oldPrice,'');
  return `<article class="en-offer-card"><div class="en-offer-media">${image?`<img src="${esc(image)}" alt="${esc(title)}" loading="lazy">`:'<span>🏷️</span>'}</div><div class="en-offer-body"><h3>${esc(title)}</h3>${description?`<p>${esc(description)}</p>`:''}${price?`<strong>${esc(price)}</strong>`:''}${oldPrice?`<del>${esc(oldPrice)}</del>`:''}</div></article>`;
}

function renderProduct(product) {
  const name=englishOnly(product.name,'Product'); const description=englishOnly(product.description,''); const image=product.imageUrl?getOptimizedImageUrl(product.imageUrl,IMAGE_SIZES.CARD):''; const price=englishOnly(product.price,'');
  return `<article class="en-product-card"><div class="en-product-media">${image?`<img src="${esc(image)}" alt="${esc(name)}" loading="lazy">`:'<span>📦</span>'}${product.isFeatured?'<b>Featured</b>':''}</div><div class="en-product-body"><h3>${esc(name)}</h3>${description?`<p>${esc(description)}</p>`:''}${price?`<strong>${esc(price)}</strong>`:''}</div></article>`;
}
...
export async function renderEnglishPlacePageV2($container,{slug,initialPlace=null}={}) {
  ...
  const coverSource=source.coverImageUrl||source.cover_image_url||assets.coverImageUrl||''; const logoSource=source.logoUrl||source.logo_url||assets.logoUrl||'';
  const cover=getOptimizedImageUrl(coverSource,IMAGE_SIZES.COVER)||'/assets/images/og-whatsapp.jpg'; const logo=getOptimizedImageUrl(logoSource,IMAGE_SIZES.LOGO); const phone=cleanPhone(source.phone); const whatsapp=cleanPhone(source.whatsapp);
  const hoursRaw=source.workingHours||source.working_hours||{}; const hours=formatWorkingHours(hoursRaw); const open=isPlaceOpen(hoursRaw); const services=englishServices(source); const address=place.displayAddress; const description=place.displayDescription; const social=source.social||{};
  const coords=source.location||{}; let mapInfo=null; try{mapInfo=resolveMapEmbedInfo(source);}catch(_){}
  let reviews=[],offers=[],products=[]; try{[reviews,offers,products]=await Promise.all([getPlaceReviews(id),getPlaceOffers(id),getPlaceProducts(id)]);}catch(_){}
  reviews=Array.isArray(reviews)?reviews:[]; offers=Array.isArray(offers)?offers:[]; products=Array.isArray(products)?products:[];
  const englishReviews=reviews.filter(r=>!/[\u0600-\u06ff]/.test(String(r.comment||''))&&!/[\u0600-\u06ff]/.test(String(r.userName||'')));
  const totalReviews=Number(source.reviewCount||source.review_count||source.reviewsCount||englishReviews.length||0); const avgRating=Number(source.rating||0)||(englishReviews.length?englishReviews.reduce((s,r)=>s+(Number(r.rating)||0),0)/englishReviews.length:0);
  const verified=isVerified(source); const availability=availabilityBadge(source.availabilityStatus||source.availability_status||source.availability);
  ...
  $container.innerHTML=`
  <div class="en-place-page">
    <div class="en-place-backbar"><div class="en-place-backbar__inner">
      <nav class="en-breadcrumbs" aria-label="Breadcrumb"><a href="/en/">Home</a><span>/</span><a href="/en/places/">Places</a><span>/</span><strong>${esc(place.displayName)}</strong></nav>
      <a class="en-back-btn" href="/en/places/">← Back</a>
    </div></div>

    <section class="en-place-hero">
      <div class="en-place-cover"><div class="en-place-cover__backdrop"></div><img src="${esc(cover)}" alt="${esc(place.displayName)} cover image" fetchpriority="high" decoding="async"><div class="en-place-cover__shade"></div></div>
      <div class="en-container">
        <div class="en-place-identity">
          <div class="en-place-logo-wrap">${logo?`<img class="en-place-logo" src="${esc(logo)}" alt="${esc(place.displayName)} logo">`:'<div class="en-place-logo en-place-logo--placeholder">🏪</div>'}</div>
          <div class="en-place-heading">
            <div class="en-place-title-line"><h1>${esc(place.displayName)}</h1>${verified?verifiedBadge():''}</div>
            <div class="en-place-meta"><span class="en-category-pill">${esc(place.displayCategory)}</span><span class="en-location-pill">📍 ${esc(place.displayArea)}</span>${open===true?'<span class="en-status-pill is-open">Open now</span>':open===false?'<span class="en-status-pill is-closed">Closed now</span>':''}${availability}</div>
            ${totalReviews?`<div class="en-rating"><span class="en-rating-stars">${stars(avgRating)}</span><b>${avgRating.toFixed(1)}</b><span>${totalReviews} reviews</span></div>`:''}
          </div>
          <div class="en-place-header-actions"><button class="en-btn en-btn--ghost" id="en-profile-card">▣ Profile Card</button><button class="en-btn en-btn--ghost" id="en-fav">${favorite?'♥ Saved':'♡ Save Place'}</button><button class="en-btn en-btn--ghost" id="en-share">↗ Share</button></div>
        </div>
        <div class="en-place-actions-bar">
          ${phone?`<a class="en-btn en-btn--primary" href="tel:${esc(phone)}">📞 Call <span>${esc(source.phone)}</span></a>`:''}
          ${whatsapp?`<a class="en-btn en-btn--whatsapp" href="${esc(buildContextualWhatsAppLink(source.whatsapp,{source:'place_page_en',placeName:place.displayName}))}" target="_blank" rel="noopener">💬 WhatsApp</a>`:''}
          ${mapUrl?`<a class="en-btn en-btn--outline" href="${esc(mapUrl)}" target="_blank" rel="noopener">🗺️ Get Directions</a>`:''}
        </div>
      </div>
    </section>

    <main class="en-container en-section"><div class="en-place-layout">
      <div class="en-place-main">
