...
function verifiedBadge() { return '<span class="en-verified-badge" title="Verified business" aria-label="Verified business"><span class="en-verified-badge__icon">✓</span><span>Verified</span></span>'; }
function availabilityBadge(status) { const s=String(status||'').toLowerCase(); if(!s)return ''; const map={available:['Available now','is-open'],busy:['Busy right now','is-busy'],unavailable:['Unavailable','is-closed']}; const item=map[s]; if(!item)return ''; return `<span class="en-status-pill ${item[1]}"><span aria-hidden="true">●</span>${item[0]}</span>`; }
function socialLinks(social = {}) { const items=[['facebook','Facebook'],['instagram','Instagram'],['tiktok','TikTok'],['youtube','YouTube'],['threads','Threads'],['x','X'],['twitter','X'],['website','Website']]; return items.filter(([key])=>social[key]).filter((item,index,arr)=>item[1]!=='X'||index===arr.findIndex(x=>x[1]==='X')).map(([key,label])=>`<a class="en-social-link" href="${esc(social[key])}" target="_blank" rel="noopener noreferrer">${label}</a>`).join(''); }
function renderHours(hours) { const list=Array.isArray(hours)?hours:[]; if(!list.length)return '<div class="en-empty-inline">Business hours are not available yet.</div>'; const names={saturday:'Saturday',sunday:'Sunday',monday:'Monday',tuesday:'Tuesday',wednesday:'Wednesday',thursday:'Thursday',friday:'Friday'}; return list.map(h=>`<div class="en-hours-row ${h.isToday?'is-today':''}"><span class="en-hours-day">${esc(names[h.key]||englishOnly(h.name,'Day'))}${h.isToday?' <em>Today</em>':''}</span><span class="en-hours-time ${h.closed?'is-closed':''}">${h.closed?'Closed':`${esc(h.open)} — ${esc(h.close)}`}</span></div>`).join(''); }
  ...
  return `<div class="en-place-page">
    ...
    <section class="en-place-hero"><div class="en-place-cover"><div class="en-place-cover__backdrop"></div><img src="${esc(cover)}" alt="${esc(place.displayName)} cover image" fetchpriority="high" decoding="async"><div class="en-place-cover__shade"></div></div>
      <div class="en-container"><div class="en-place-identity"><div class="en-place-logo-wrap">${logo ? `<img class="en-place-logo" src="${esc(logo)}" alt="${esc(place.displayName)} logo">` : '<div class="en-place-logo en-place-logo--placeholder">🏪</div>'}</div>
        <div class="en-place-heading"><div class="en-place-title-line"><h1>${esc(place.displayName)}</h1>${verified ? verifiedBadge() : ''}</div><div class="en-place-meta"><span class="en-category-pill">${esc(place.displayCategory)}</span><span class="en-location-pill">📍 ${esc(place.displayArea)}</span>${open === true ? '<span class="en-status-pill is-open">Open now</span>' : open === false ? '<span class="en-status-pill is-closed">Closed now</span>' : ''}${availability}${renderPlaceViewsBadgeHTML(source, { isEn: true, compact: true })}</div>${totalReviews ? `<div class="en-rating"><span class="en-rating-stars">${stars(avgRating)}</span><b>${avgRating.toFixed(1)}</b><span>${totalReviews} reviews</span></div>` : ''}</div>
