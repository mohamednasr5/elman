...
export async function renderEnglishNowPage($container) {
  ...
  $container.innerHTML = `
    <div class="search-page-header"><div class="container text-center">
      <h1 style="color:#fff;font-size:var(--font-size-3xl);margin-bottom:var(--space-2)">🟢 Places Open Right Now</h1>
      <p style="color:rgba(255,255,255,0.85);max-width:560px;margin:0 auto;font-size:14px">Find pharmacies, clinics, restaurants, supermarkets, and emergency services currently open in El Manzala & El Matariya.</p>
    </div></div>
    <div class="container section">
      <div class="filter-bar">
        <div style="position:relative;flex:1;min-width:200px"><input type="search" id="now-search-filter" class="form-input" placeholder="Search open places..." style="margin:0" /></div>
        <select id="now-category-filter" class="form-select" style="max-width:200px"><option value="">All Categories</option></select>
        <select id="now-area-filter" class="form-select" style="max-width:200px"><option value="">All Towns & Villages</option>${Object.values(VILLAGE_NAMES_EN).map(en => `<option value="${escAttr(en)}">${escHtml(en)}</option>`).join('')}</select>
      </div>
      <div class="search-stats-bar" style="margin-bottom:var(--space-4);color:var(--text-secondary);font-size:var(--font-size-sm)"><span id="now-count-label">Checking open places...</span></div>
      <div class="grid grid-4" id="now-places-grid">${Array(8).fill(renderEnglishPlaceCardSkeleton()).join('')}</div>
    </div>
  `;
  ...
  if (catSelect && categories) categories.forEach(c => {
  ...
  const openList = (places || []).filter(p => {
    if (!p) return false;
  ...
  function applyFilter() {
    ...
    if (!filtered.length) {
      grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1;text-align:center;padding:56px 16px"><div class="empty-state__icon">🌙</div><h3 class="empty-state__title">No open places matching your filter</h3><p class="empty-state__text">Try checking other categories, towns, or reset your filters.</p></div>`;
      return;
    }
