...
async function renderQuranSurah(container){
 ...
 try{
  ...
  '<div class="qr-surah-picker"><label for="qr-surah-select">اختيار السورة</label><select id="qr-surah-select" class="qr-select" aria-label="اختيار السورة">'+meta.map(m=>'<option value="'+m.number+'"'+(m.number===n?' selected':'')+'>'+m.number+' — سورة '+esc(m.name)+'</option>').join('')+'</select></div><div class="qr-actions"><select id="qr-font" class="qr-select"><option value="1">حجم الخط: متوسط</option><option value="1.15">حجم الخط: كبير</option><option value=".9">حجم الخط: صغير</option></select>'+ 
  ...
  surahSelect.onchange=e=>{const target=Math.min(114,Math.max(1,Number(e.target.value)||1));if(target!==n)location.href='quran-surah.html?surah='+target};
