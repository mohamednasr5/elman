...
async function renderQuranSurah(container){
 ...
 try{
  ...
  const goSurah=target=>{target=Math.min(114,Math.max(1,Number(target)||1));if(target!==n)location.href='quran-surah.html?surah='+target};
  surahSelect.onchange=e=>goSurah(e.target.value);
  box.querySelector('#qr-prev-surah').onclick=()=>goSurah(n-1);
  box.querySelector('#qr-next-surah').onclick=()=>goSurah(n+1);
