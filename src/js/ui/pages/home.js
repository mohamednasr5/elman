import{getCategories as pe,getPublishedPlaces as Q,getActiveOffers as J,getAds as me,getCached as w,FALLBACK_CATEGORIES as R}from"../../core/db.js";import{WORKER_URL as K}from"../../core/firebase.js";import"../../core/state.js";import{renderPlaceCard as he,renderPlaceCardSkeleton as ge}from"../components/PlaceCard.js";import{isAtmPlace as ve}from"../../utils/atm.js";import{mountSponsoredShowcase as ue,isPlaceSponsored as fe}from"../components/SponsoredShowcase.js";import{formatPrice as Y,calcDiscount as be,arabicMatch as X}from"../../utils/arabic.js";import{daysUntil as ye}from"../../utils/date.js";import{getCurrentUser as q}from"../../core/auth.js";import{openManzalaVoiceAssistantModal as _e}from"../../services/voice.service.js";import{executeFastSearch as we,warmupSearchEngine as z}from"../../services/search-engine.service.js";import{getCategorySvg as ee}from"../../utils/professions-data.js";import{getCategoryVisualMeta as xe,renderCategoryCardIcon as Ce}from"../../utils/category-visual.js";import{resolveDeliveryVehicle as $e}from"../../utils/delivery-vehicle.js";const Je={pharmacy:{emoji:"\u{1F48A}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},supermarket:{emoji:"\u{1F6D2}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},paint:{emoji:"\u{1F3A8}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},herbs:{emoji:"\u{1F33F}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},doctor:{emoji:"\u{1F468}\u200D\u2695\uFE0F",color:"rgba(41,128,185,0.1)",border:"#2980B9"},plumbing:{emoji:"\u{1F527}",color:"rgba(52,73,94,0.1)",border:"#52596E"},plumber:{emoji:"\u{1FAA0}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpenter:{emoji:"\u{1FA9A}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},tiler:{emoji:"\u{1F9F1}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},painter:{emoji:"\u{1F58C}\uFE0F",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},electrician:{emoji:"\u26A1",color:"rgba(243,156,18,0.1)",border:"#F39C12"},"ac-technician":{emoji:"\u2744\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},blacksmith:{emoji:"\u{1F6E0}\uFE0F",color:"rgba(52,73,94,0.1)",border:"#52596E"},alumital:{emoji:"\u{1FA9F}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},mechanic:{emoji:"\u{1F529}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},upholsterer:{emoji:"\u{1F6CB}\uFE0F",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},feed:{emoji:"\u{1F33E}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},poultry:{emoji:"\u{1F357}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},bakery:{emoji:"\u{1F35E}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},vegetables:{emoji:"\u{1F96C}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},antiques:{emoji:"\u{1F3FA}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},electronics:{emoji:"\u{1F4FA}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpet:{emoji:"\u{1F9F6}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},mattress:{emoji:"\u{1F6CF}\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},china:{emoji:"\u{1F37D}\uFE0F",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},electrical:{emoji:"\u{1F4A1}",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},roastery:{emoji:"\u{1F95C}",color:"rgba(101,67,33,0.1)",border:"#654321"},phones:{emoji:"\u{1F4F1}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},grocery:{emoji:"\u{1F3EA}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},hypermarket:{emoji:"\u{1F3EC}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"},delivery:{emoji:"\u{1F680}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"}},Ke={emoji:"\u{1F3EA}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"};export async function renderHomePage(n,{user:i}={}){const t=document.getElementById("hero-section-static");if(!!!t)n.innerHTML=ce(),ne();else{t.removeAttribute("id");const e=[];let c=t.nextSibling;for(;c;)e.push(c),c=c.nextSibling;e.forEach(v=>v.parentNode&&v.parentNode.removeChild(v));const d=document.createElement("div");d.innerHTML=ce();const h=d.querySelector(".hero");if(h){if(!t.querySelector(".hero-heritage-decor")){const v=h.querySelector(".hero-heritage-decor");v&&t.insertAdjacentElement("afterbegin",v)}h.remove()}for(;d.firstChild;)n.appendChild(d.firstChild);ne()}T();try{const e=w("categories_all"),c=w("published_100_");if(Array.isArray(e)&&e.length>0&&(ae(e),oe(e)),Array.isArray(c)&&c.length>0){const d=q()||i;T(c);const h=U(c,d?.uid);F(h.slice(0,8)),se(c.length,e?.length||31),z(c,e||[])}}catch{}let s=[],r=[],l=[],u=[];try{const[e,c,d,h]=await Promise.allSettled([pe(),Q({limit:100}),J(8),me("homepage")]);s=e.status==="fulfilled"&&Array.isArray(e.value)&&e.value.length?e.value:w("categories_all")||R||[],r=c.status==="fulfilled"&&Array.isArray(c.value)&&c.value.length?c.value:w("published_100_")||[],l=d.status==="fulfilled"&&Array.isArray(d.value)?d.value:w("offers_active_8")||[],u=h.status==="fulfilled"&&Array.isArray(h.value)?h.value:w("ads_homepage")||[]}catch(e){console.warn("[Home] Data load non-fatal warning:",e),s=w("categories_all")||R||[],r=w("published_100_")||[],l=w("offers_active_8")||[],u=w("ads_homepage")||[]}const m=q()||i;if(typeof window<"u"&&Array.isArray(r)&&r.length>0){window._placesRegistry=window._placesRegistry||new Map;for(const e of r){if(!e)continue;const c=String(e.slug||e.id||e._key||"").toLowerCase().trim();c&&(window._placesRegistry.set(c,e),e.id&&window._placesRegistry.set(String(e.id).toLowerCase().trim(),e),e.slug&&window._placesRegistry.set(String(e.slug).toLowerCase().trim(),e))}}try{s&&s.length&&ae(s)}catch(e){console.warn("[Home] renderCategories err:",e)}try{T(r)}catch(e){console.warn("[Home] initHomeVerifiedShowcase err:",e)}try{const e=U(r,m?.uid);F(e.slice(0,8))}catch(e){console.warn("[Home] renderLatestPlaces err:",e)}try{l&&l.length&&re(l)}catch(e){console.warn("[Home] renderOffers err:",e)}try{const e=(r||[]).filter(c=>{if(!c)return!1;if(c.deliveryType||c.categoryId?.includes("delivery"))return!0;const d=String(c.name||"").toLowerCase();return/توكتوك|تاكسي|شانجي|اتوبيس|توصيل|دليفري|وصلي/i.test(d)?!/صيدلية|مطعم|كشري|حلواني|سوبر\s*ماركت|هايبر/i.test(d):!1});e.length&&Le(e)}catch(e){console.warn("[Home] renderDeliveryServices err:",e)}try{u&&u.length&&Se(u)}catch(e){console.warn("[Home] renderAds err:",e)}try{se(r.length||0,s?.length||31)}catch{}try{z(r,s||[])}catch{}try{oe(s||[])}catch{}try{Me()}catch{}Promise.resolve().then(()=>{try{import("../components/WhoIsAvailableNow.js").then(({renderWhoIsAvailableNow:e})=>{const c=document.getElementById("home-oncall-craftsmen-container");c&&e(c)}).catch(e=>console.warn("[Home] WhoIsAvailableNow load err:",e))}catch{}try{import("../components/ServiceRequestsSection.js").then(({renderServiceRequestsSection:e})=>{const c=document.getElementById("home-service-requests-container");c&&e(c,{limit:4,showHero:!1,isCompact:!0})}).catch(e=>console.warn("[Home] ServiceRequestsSection load err:",e))}catch{}try{import("../components/AroundMeRadar.js").then(({mountAroundMeRadar:e})=>{e("home-around-me-container")}).catch(e=>console.warn("[Home] AroundMeRadar load err:",e))}catch{}try{ue("home-sponsored-container",r,{title:"\u0623\u0645\u0627\u0643\u0646 \u0648\u0625\u0639\u0644\u0627\u0646\u0627\u062A \u0645\u0645\u064A\u0632\u0629 \u0641\u064A \u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",subtitle:"\u0623\u0646\u0634\u0637\u0629 \u062A\u062C\u0627\u0631\u064A\u0629 \u0648\u062E\u062F\u0645\u0627\u062A \u0645\u0648\u0635\u0649 \u0628\u0647\u0627 \u0648\u0645\u0639\u062A\u0645\u062F\u0629 \u0641\u064A \u0627\u0644\u0645\u062F\u064A\u0646\u0629",maxVisible:4})}catch(e){console.warn("[Home] SponsoredShowcase mount err:",e)}try{import("../components/WideAdsBanner.js").then(({mountWideAdsBanner:e})=>e("wide-ads-banner")).catch(e=>console.warn("[Home] WideAdsBanner load err:",e))}catch{}try{Be()}catch{}}),typeof window<"u"&&!window._homeRealtimeSyncAttached&&(window._homeRealtimeSyncAttached=!0,window.addEventListener("manzala:realtime_sync",async e=>{try{const[c,d]=await Promise.all([Q({limit:100,forceFresh:!0}).catch(()=>[]),J(8).catch(()=>[])]);if(c&&c.length){const h=q()||userArg,v=U(c,h?.uid);F(v.slice(0,8)),T(c),z(c,w("categories_all")||[])}d&&d.length&&re(d)}catch(c){console.warn("[Home Realtime Sync Error]:",c)}}))}function U(n,i=null,t=!0){const a=new Set,s=[],r=[];[...n].sort((h,v)=>{const f=Number(h.createdAt)||Number(h.updatedAt)||0;return(Number(v.createdAt)||Number(v.updatedAt)||0)-f}).forEach(h=>{const v=h._key||h.id;a.has(v)||(a.add(v),fe(h)?s.push(h):r.push(h))});const u=ie(s),m=Math.min(r.length,45),e=r.slice(0,m),c=r.slice(m),d=ie(e);return[...u,...d,...c]}function ae(n){const i=document.getElementById("categories-grid");if(!i)return;const t=Array.isArray(n)&&n.length>0?n:w("categories_all")||R||[];if(!t||!t.length)return i.querySelector(".category-card"),void 0;i.innerHTML=t.map(a=>{const s=a.slug||a._key||a.id||"",r=xe(a),l=Ce(a,{size:40});return`
    ...
    `}).join("")}const te="manzala_verified_showcase_v1";let I=null,C=[],P=0;const ke=[{id:"p_1788703900620_oae8ka",slug:"mtam-basl-wbaha-llmakwlat-albhrya",name:"\u0645\u0637\u0639\u0645 \u0628\u0627\u0633\u0644 \u0648\u0628\u0627\u0647\u0649 \u0644\u0644\u0645\u0623\u0643\u0648\u0644\u0627\u062A \u0627\u0644\u0628\u062D\u0631\u064A\u0629",area:"\u0627\u0644\u0645\u0637\u0631\u064A\u0629 \u062F\u0642\u0647\u0644\u064A\u0629",address:"\u0627\u0644\u0645\u0637\u0631\u064A\u0629 - \u0634 \u0627\u0644\u062B\u0648\u0631\u0629",phone:"01062944644",whatsapp:"01062944644",category:"\u0645\u0637\u0627\u0639\u0645 \u0648\u0623\u0633\u0645\u0627\u0643",cover:"https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"-P03LX9MledW_z7QfyHO",slug:"-P03LX9MledW_z7QfyHO",name:"\u0627\u0644\u062D\u0633\u0646 \u0644\u0635\u064A\u0627\u0646\u0629 \u0627\u0644\u0647\u0648\u0627\u062A\u0641 \u0627\u0644\u0645\u062D\u0645\u0648\u0644\u0629",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0634\u0627\u0631\u0639 \u0627\u0644\u0628\u062D\u0631",address:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0634\u0627\u0631\u0639 \u0627\u0644\u0628\u062D\u0631 \u0623\u0645\u0627\u0645 \u0627\u0644\u0628\u0646\u0643",phone:"01026046049",whatsapp:"01026046049",category:"\u0635\u064A\u0627\u0646\u0629 \u0648\u0645\u0648\u0628\u0627\u064A\u0644",cover:"https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"p_1788801925745_vuxmjs",slug:"mtbkh-eyma-llaakl-albyty",name:"\u0645\u0637\u0628\u062E \u0625\u064A\u0645\u0649 \u0644\u0644\u0623\u0643\u0644 \u0627\u0644\u0628\u064A\u062A\u064A",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0637\u0631\u064A\u0642 \u0627\u0644\u0645\u0646\u0635\u0648\u0631\u0629",address:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0637\u0631\u064A\u0642 \u0627\u0644\u0645\u0646\u0635\u0648\u0631\u0629 \u0627\u0644\u0631\u0626\u064A\u0633\u064A",phone:"01090123456",whatsapp:"01090123456",category:"\u0623\u0643\u0644 \u0628\u064A\u062A\u064A \u0648\u062D\u0644\u0648\u064A\u0627\u062A",cover:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1},{id:"-P0hEa0K6ZfAM65O27G9",slug:"kwafyr-mnh-asad",name:"\u0643\u0648\u0627\u0641\u064A\u0631 \u0645\u0646\u0647 \u0623\u0633\u0639\u062F",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u062D\u064A \u0627\u0644\u0633\u0644\u0627\u0645",address:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u062D\u064A \u0627\u0644\u0633\u0644\u0627\u0645",phone:"01099887766",whatsapp:"01099887766",category:"\u0628\u064A\u0648\u062A\u064A \u0648\u0643\u0648\u0627\u0641\u064A\u0631",cover:"https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1}];function T(n=null){const i=document.getElementById("home-verified-cards-grid"),t=document.getElementById("home-verified-status-text");if(!i)return;let a=null;try{const m=localStorage.getItem(te);if(m){const e=JSON.parse(m);Array.isArray(e)&&e.length>0&&(a=e)}}catch{}if(Array.isArray(n)&&n.length>0){const m=n.filter(e=>e&&e.isVerified&&!ve(e)).map(e=>({...e,id:e.id||e._key,slug:e.slug||e.id,name:e.name,area:e.area||"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",address:e.address||"",phone:e.phone||"",whatsapp:e.whatsapp||"",logoUrl:e.logoUrl||"",coverImageUrl:e.coverImageUrl||e.gallery&&e.gallery[0]||"",category:e.categoryName||e.customCategory||e.categoryId||"\u0646\u0634\u0627\u0637 \u062A\u062C\u0627\u0631\u064A",cover:e.coverImageUrl||e.logoUrl||e.gallery&&e.gallery[0]||"/assets/images/og-whatsapp.jpg",isSponsored:!!(e.isSponsored&&(!e.sponsoredUntil||e.sponsoredUntil>Date.now()))}));if(m.length>0){C=m;try{localStorage.setItem(te,JSON.stringify(m))}catch{}}}else C.length||(a&&a.length>0?C=a:C=[...ke]);if(typeof window<"u"&&Array.isArray(C)){window._placesRegistry=window._placesRegistry||new Map;for(const m of C){if(!m)continue;const e=String(m.slug||m.id||"").toLowerCase().trim();e&&(window._placesRegistry.set(e,m),m.slug&&window._placesRegistry.set(String(m.slug).toLowerCase().trim(),m),m.id&&window._placesRegistry.set(String(m.id).toLowerCase().trim(),m))}}const s=["\u{1F947} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #1","\u{1F948} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #2","\u{1F949} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #3","\u{1F396}\uFE0F \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #4"];function r(m){i.innerHTML=m.map((e,c)=>{const d=e.slug||e.id||"";return`
      <article class="fair-place-card" data-card-index="${c}"
               ...
               onclick="window.__openPlaceCard ? window.__openPlaceCard(this, '${o(d)}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(d)}')"
               ontouchstart="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${o(d)}', this)"
               onpointerdown="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${o(d)}', this)"
               onmouseenter="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${o(d)}', this)"
               style="cursor:pointer">
        <span class="fair-place-card__rank">${s[c]||`\u{1F396}\uFE0F \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #${c+1}`}</span>
        ...
        <div class="fair-place-card__body">
          <h3 class="fair-place-card__title" title="${o(e.name)}">${_(e.name)}</h3>
          <div class="fair-place-card__meta">
            <span>\u{1F4CD} ${_(e.area)}</span>
            <span>\u{1F3F7}\uFE0F ${_(e.category)}</span>
          </div>
          <a href="/place.html?slug=${encodeURIComponent(d)}" class="fair-place-card__link" onclick="event.preventDefault(); window.__openPlaceCard ? window.__openPlaceCard(this, '${o(d)}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(d)}')">\u0639\u0631\u0636 \u0628\u0637\u0627\u0642\u0629 \u0627\u0644\u0645\u0643\u0627\u0646 \u2197</a>
        </div>
      </article>
    `}).join("")}const l=C.length,u=[];for(let m=0;m<Math.min(4,l);m++)u.push(C[(P+m)%l]);if(r(u),I&&(clearInterval(I),I=null),l>=2){let m=1;I=setInterval(()=>{i.querySelectorAll(".fair-place-card").forEach(c=>c.classList.add("anim-swap")),setTimeout(()=>{P=(P+1)%C.length,m++;const c=[],d=C.length;for(let h=0;h<Math.min(4,d);h++)c.push(C[(P+h)%d]);r(c),t&&(t.innerHTML=`\u{1F7E2} <b>\u062A\u0645 \u062A\u062F\u0648\u064A\u0631 \u0627\u0644\u0635\u062F\u0627\u0631\u0629 \u062A\u0644\u0642\u0627\u0626\u064A\u0627\u064B (${m}):</b> \u062A\u062A\u063A\u064A\u0631 \u0627\u0644\u0645\u0631\u0627\u0643\u0632 \u062F\u0648\u0631\u064A\u0627\u064B \u0644\u0636\u0645\u0627\u0646 \u062A\u0643\u0627\u0641\u0624 \u0646\u0633\u0628 \u0627\u0644\u0645\u0634\u0627\u0647\u062F\u0629 \u0644\u0643\u0627\u0641\u0629 \u0627\u0644\u0623\u0645\u0627\u0643\u0646 \u0627\u0644\u0645\u0648\u062B\u0642\u0629!`)},300)},4500)}}function ie(n){const i=[...n];for(let t=i.length-1;t>0;t--){const a=Math.floor(Math.random()*(t+1));[i[t],i[a]]=[i[a],i[t]]}return i}function F(n){const i=document.getElementById("latest-places-grid");if(i){if(!n||!n.length){if(i.querySelector(".fair-place-card")||i.querySelector(".place-card"))return;i.innerHTML=`
  ...
  `}).join("")}}function Se(n){const i=document.getElementById("ads-container");if(!i||!n||!n.length)return;const t=n.filter(a=>a&&(a.imageUrl||a.image_url));if(!t.length){i.innerHTML="";return}i.innerHTML=t.map(a=>{let s=(a.link||"#").trim();s.startsWith("place.html")&&(s="/"+s);const r=a.id||a._id||"",l=a.title||"\u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632",u=a.imageUrl||a.image_url||"";return`
      <a href="${o(s)}" class="ad-banner" target="_blank" rel="noopener noreferrer sponsored" aria-label="${o(l)}" data-ad-id="${o(r)}">
    ...
    `}).join(""),i.querySelectorAll(".ad-banner").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-ad-id");if(s&&K)try{fetch(`${K}/api/ads/track-click`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:s}),keepalive:!0}).catch(()=>{})}catch{}})})}class Ae{constructor(){this.ctx=null,this.lastTickTime=0}init(){if(!this.ctx&&typeof window<"u"){const i=window.AudioContext||window.webkitAudioContext;i&&(this.ctx=new i)}}playTick(i=550){try{if(!this.ctx||this.ctx.state!=="running")return;const t=this.ctx.currentTime;if(t-this.lastTickTime<.038)return;this.lastTickTime=t;const a=this.ctx.createOscillator(),s=this.ctx.createGain();a.type="sine",a.frequency.setValueAtTime(i,t),a.frequency.exponentialRampToValueAtTime(i*1.35,t+.018),s.gain.setValueAtTime(.04,t),s.gain.exponentialRampToValueAtTime(1e-4,t+.022),a.connect(s),s.connect(this.ctx.destination),a.start(t),a.stop(t+.028)}catch{}}playDoneChime(){try{if(!this.ctx||this.ctx.state!=="running")return;const i=this.ctx.currentTime;[1046.5,1318.51,1567.98,2093].forEach((a,s)=>{const r=this.ctx.createOscillator(),l=this.ctx.createGain();r.type="triangle",r.frequency.setValueAtTime(a,i+s*.055),l.gain.setValueAtTime(.06,i+s*.055),l.gain.exponentialRampToValueAtTime(1e-4,i+s*.055+.32),r.connect(l),l.connect(this.ctx.destination),r.start(i+s*.055),r.stop(i+s*.055+.35)})}catch{}}}const A=new Ae;function se(n,i){const t=document.getElementById("stats-bar");if(!t)return;const a=5e4,s=Math.max(15e3,Number(n)||0),r=12e3,l=Math.max(124,Number(i)||0),u=55;t.innerHTML=`
  ...
  `,Ee(t)}function Ee(n){let i=!1,t=!1;function a(){if(t)return;t=!0;const s=n.querySelectorAll(".stats-bar__value[data-target]");if(!s.length){t=!1;return}const r=1800,l=performance.now();s.forEach(e=>{e.classList.remove("stats-done"),e.classList.add("stats-counting")});let u=-1;function m(e){const c=e-l,d=Math.min(1,c/r),h=1-Math.pow(1-d,3);s.forEach(v=>{const f=parseInt(v.getAttribute("data-target"),10)||0,p=v.getAttribute("data-prefix")||"",b=v.getAttribute("data-suffix")||"",$=Math.floor(h*f);if(v.textContent=`${p}${$.toLocaleString("en-US")}${b}`,$!==u){u=$;const x=420+$/Math.max(1,f)*430;A.playTick(x)}}),d<1?requestAnimationFrame(m):(s.forEach(v=>{const f=parseInt(v.getAttribute("data-target"),10)||0,p=v.getAttribute("data-prefix")||"",b=v.getAttribute("data-suffix")||"";v.textContent=`${p}${f.toLocaleString("en-US")}${b}`,v.classList.remove("stats-counting"),v.classList.add("stats-done")}),A.playDoneChime(),t=!1)}requestAnimationFrame(m)}if("IntersectionObserver"in window){const s=new IntersectionObserver(r=>{r.forEach(l=>{l.isIntersecting&&!i&&(i=!0,a(),s.disconnect())})},{threshold:.15});s.observe(n)}else a();n.querySelectorAll(".stats-interactive-item").forEach(s=>{s.addEventListener("click",()=>{A.init(),A.ctx&&A.ctx.state==="suspended"&&A.ctx.resume().catch(()=>{}),a()})})}function oe(n){const i=document.getElementById("hero-search-glow-wrap"),t=document.getElementById("hero-search-input"),a=document.getElementById("hero-search-btn"),s=document.getElementById("hero-search-clear"),r=document.getElementById("hero-live-dropdown");if(!t)return;r&&!document.getElementById("hero-live-list")&&(r.innerHTML=`
    ...
    `);const l=document.getElementById("hero-live-list"),u=document.getElementById("hero-live-count"),m=document.getElementById("hero-live-all-btn"),e=document.getElementById("hero-quick-cats");e&&Array.isArray(n)&&n.length>0&&(e.innerHTML=n.slice(0,10).map(p=>{const b=p.slug||p._key||p.id||"",$=ee(b||p.name,18);return`
        <a href="category.html?slug=${encodeURIComponent(b)}" class="hero__quick-cat">
          ${$||p.icon||"\u{1F3EA}"} ${_(p.name)}
        </a>
      `}).join(""));function c(){const p=t.value.trim();p&&(window.location.href=`search.html?q=${encodeURIComponent(p)}`)}a?.addEventListener("click",p=>{p.stopPropagation(),c()}),t.addEventListener("keydown",p=>{p.key==="Enter"?c():p.key==="Escape"&&r?.classList.remove("visible")}),s?.addEventListener("click",p=>{p.stopPropagation(),t.value="",s.classList.remove("visible"),r?.classList.remove("visible"),l&&(l.innerHTML=""),t.focus()});function d(){!r||!l||(u&&(u.textContent="\u0645\u0642\u062A\u0631\u062D\u0627\u062A"),l.innerHTML=`
    ...
    `,l.querySelectorAll(".hero-live-suggestion-chip").forEach(p=>{p.addEventListener("click",b=>{b.preventDefault(),b.stopPropagation();const $=p.getAttribute("data-q")||"";t.value=$,t.dispatchEvent(new Event("input",{bubbles:!0})),t.focus()})}),r.classList.add("visible"))}t.addEventListener("focus",()=>{t.value.trim().length>=1&&l?.children.length>0?r?.classList.add("visible"):t.value.trim()||d()}),t.addEventListener("click",()=>{t.value.trim()||d()});let h=null,v=0;t.addEventListener("input",()=>{const p=t.value.trim();if(s?.classList.toggle("visible",p.length>0),m&&(m.href=`search.html?q=${encodeURIComponent(p)}`),!p){r?.classList.remove("visible"),l&&(l.innerHTML="");return}const b=p.length<=2?0:35;clearTimeout(h),h=setTimeout(async()=>{const $=++v;try{let V=function(y,g){if(!y)return"";if(!g)return _(y);const L=g.trim().split(/\s+/).filter(Boolean);if(!L.length)return _(y);const E=L.map(k=>k.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")),M=new RegExp(`(${E.join("|")})`,"gi");return _(y).replace(M,'<span class="search-highlight">$1</span>')};const x=await we(p,{limit:6});if($!==v||!r||!l)return;if(!x||x.length===0){u&&(u.textContent="0"),l.innerHTML=`
          ...
          `,r.classList.add("visible");return}u&&(u.textContent=String(x.length));let O="";x.matchingCategories&&x.matchingCategories.length>0&&(O=`
            <div class="hero-live-matched-cats">
              ...
              <div class="hero-live-matched-cats__chips">
                ${x.matchingCategories.map(y=>{const g=y.slug||y.id||"",L=ee(g||y.name,16);return`
                    <a href="category.html?slug=${encodeURIComponent(g)}" class="hero-live-matched-cat-chip" onclick="event.stopPropagation()">
                      ${L||y.icon||"\u{1F3EA}"}
                      <span>${_(y.name)}</span>
                    </a>
                  `}).join("")}
          ...
          `),l.innerHTML=O+x.map(y=>{const g=y.raw||y,L=g.name||"\u0645\u0643\u0627\u0646 \u0628\u0627\u0644\u062F\u0644\u064A\u0644",E=g.categoryName||y.category||"",M=g.area||g.address||"\u0645\u062F\u064A\u0646\u0629 \u0627\u0644\u0645\u0646\u0632\u0644\u0629",k=g.slug||g.id||"",B=g.photoURL||g.logo||g.coverURL||g.coverImageUrl||g.logoUrl||"",le=g.isVerified||!1,Z=g.isOpen!==void 0?g.isOpen:!0,N=(L.trim()[0]||"\u0645").toUpperCase(),j=(g.phone||"").trim(),W=(g.whatsapp||g.phone||"").trim(),S=W?W.replace(/[^0-9]/g,""):"",H=S?S.startsWith("2")?S:S.startsWith("0")?"2"+S:"20"+S:"";let G="";if((j||H)&&(G=`
            ...
            `),typeof window<"u"&&window._placesRegistry&&k){const de=String(k).toLowerCase().trim();window._placesRegistry.set(de,g),g.slug&&window._placesRegistry.set(String(g.slug).toLowerCase().trim(),g),g.id&&window._placesRegistry.set(String(g.id).toLowerCase().trim(),g)}return`
            <a href="/place.html?slug=${encodeURIComponent(k)}" class="hero-live-dropdown__item" role="option"
               data-place-id="${o(g.id||k)}"
               data-place-slug="${o(k)}"
               data-name="${o(L)}"
               data-phone="${o(g.phone||"")}"
               data-whatsapp="${o(g.whatsapp||"")}"
               data-area="${o(M)}"
               data-address="${o(g.address||"")}"
               data-cover="${o(g.coverImageUrl||B)}"
               data-logo="${o(g.logoUrl||B)}"
               data-category="${o(E)}"
              ...
              <div class="hero-live-avatar">
                ${B?`<img src="${o(B)}" alt="${o(L)}" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML='<div class=\\'hero-live-avatar-fallback\\'>${N}</div>'"/>`:`<div class="hero-live-avatar-fallback">${N}</div>`}
              </div>
              <div class="hero-live-content">
                <div class="hero-live-title-row">
                  <span class="hero-live-name">${V(L,p)}</span>
                  ${le?'<span class="hero-live-verified" title="\u0645\u0643\u0627\u0646 \u0645\u0648\u062B\u0642">\u2713</span>':""}
                </div>
                <div class="hero-live-meta-row">
                  ${E?`<span class="hero-live-cat">${V(E,p)}</span>`:""}
