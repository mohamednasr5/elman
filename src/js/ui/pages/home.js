import{getCategories as G,getPublishedPlaces as J,getActiveOffers as K,getAds as Y}from"../../core/db.js";import{WORKER_URL as R}from"../../core/firebase.js";import"../../core/state.js";import{renderPlaceCard as X,renderPlaceCardSkeleton as ee}from"../components/PlaceCard.js";import{isAtmPlace as te}from"../../utils/atm.js";import{mountSponsoredShowcase as ae,isPlaceSponsored as ie}from"../components/SponsoredShowcase.js";import{formatPrice as F,calcDiscount as re,arabicMatch as D}from"../../utils/arabic.js";import{daysUntil as oe}from"../../utils/date.js";import{getCurrentUser as se}from"../../core/auth.js";import{openManzalaVoiceAssistantModal as ne}from"../../services/voice.service.js";import{executeFastSearch as ce}from"../../services/search-engine.service.js";import{getCategorySvg as le}from"../../utils/professions-data.js";import{getCategoryVisualMeta as de,renderCategoryCardIcon as pe}from"../../utils/category-visual.js";const Ve={pharmacy:{emoji:"\u{1F48A}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},supermarket:{emoji:"\u{1F6D2}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},paint:{emoji:"\u{1F3A8}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},herbs:{emoji:"\u{1F33F}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},doctor:{emoji:"\u{1F468}\u200D\u2695\uFE0F",color:"rgba(41,128,185,0.1)",border:"#2980B9"},plumbing:{emoji:"\u{1F527}",color:"rgba(52,73,94,0.1)",border:"#52596E"},plumber:{emoji:"\u{1FAA0}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpenter:{emoji:"\u{1FA9A}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},tiler:{emoji:"\u{1F9F1}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},painter:{emoji:"\u{1F58C}\uFE0F",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},electrician:{emoji:"\u26A1",color:"rgba(243,156,18,0.1)",border:"#F39C12"},"ac-technician":{emoji:"\u2744\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},blacksmith:{emoji:"\u{1F6E0}\uFE0F",color:"rgba(52,73,94,0.1)",border:"#52596E"},alumital:{emoji:"\u{1FA9F}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},mechanic:{emoji:"\u{1F529}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},upholsterer:{emoji:"\u{1F6CB}\uFE0F",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},feed:{emoji:"\u{1F33E}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},poultry:{emoji:"\u{1F357}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},bakery:{emoji:"\u{1F35E}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},vegetables:{emoji:"\u{1F96C}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},antiques:{emoji:"\u{1F3FA}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},electronics:{emoji:"\u{1F4FA}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpet:{emoji:"\u{1F9F6}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},mattress:{emoji:"\u{1F6CF}\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},china:{emoji:"\u{1F37D}\uFE0F",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},electrical:{emoji:"\u{1F4A1}",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},roastery:{emoji:"\u{1F95C}",color:"rgba(101,67,33,0.1)",border:"#654321"},phones:{emoji:"\u{1F4F1}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},grocery:{emoji:"\u{1F3EA}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},hypermarket:{emoji:"\u{1F3EC}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"},delivery:{emoji:"\u{1F680}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"}},Ze={emoji:"\u{1F3EA}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"};export async function renderHomePage(n,{user:r}={}){const e=document.getElementById("hero-section-static");if(!!!e)n.innerHTML=O(),N();else{e.removeAttribute("id");const i=[];let o=e.nextSibling;for(;o;)i.push(o),o=o.nextSibling;i.forEach(c=>c.parentNode&&c.parentNode.removeChild(c));const d=document.createElement("div");d.innerHTML=O();const h=d.querySelector(".hero");if(h){if(!e.querySelector(".hero-heritage-decor")){const c=h.querySelector(".hero-heritage-decor");c&&e.insertAdjacentElement("afterbegin",c)}h.remove()}for(;d.firstChild;)n.appendChild(d.firstChild);N()}Z();try{const[i,o,d,h]=await Promise.all([G(),J({limit:100}),K(8),Y("homepage")]),c=se()||r,a=o||[];if(typeof window<"u"&&Array.isArray(a)){window._placesRegistry=window._placesRegistry||new Map;for(const l of a){if(!l)continue;const g=String(l.slug||l.id||l._key||"").toLowerCase().trim();g&&(window._placesRegistry.set(g,l),l.id&&window._placesRegistry.set(String(l.id).toLowerCase().trim(),l),l.slug&&window._placesRegistry.set(String(l.slug).toLowerCase().trim(),l))}}he(i||[]),Z(a);const v=me(a,c?.uid);ue(v.slice(0,8)),fe(d||[]),be(a.filter(l=>l.categoryId?.includes("delivery")||l.deliveryType)),ye(h||[]),we(a.length||0,i?.length||31),Ce(i||[]),ke(),Promise.resolve().then(()=>{import("../components/WhoIsAvailableNow.js").then(({renderWhoIsAvailableNow:l})=>{const g=document.getElementById("home-oncall-craftsmen-container");g&&l(g)}).catch(()=>{}),import("../components/ServiceRequestsSection.js").then(({renderServiceRequestsSection:l})=>{const g=document.getElementById("home-service-requests-container");g&&l(g,{limit:4,showHero:!0,isCompact:!0})}).catch(()=>{}),import("../components/AroundMeRadar.js").then(({mountAroundMeRadar:l})=>{l("home-around-me-container")}).catch(()=>{}),ae("home-sponsored-container",a,{title:"\u0623\u0645\u0627\u0643\u0646 \u0648\u0625\u0639\u0644\u0627\u0646\u0627\u062A \u0645\u0645\u064A\u0632\u0629 \u0641\u064A \u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",subtitle:"\u0623\u0646\u0634\u0637\u0629 \u062A\u062C\u0627\u0631\u064A\u0629 \u0648\u062E\u062F\u0645\u0627\u062A \u0645\u0648\u0635\u0649 \u0628\u0647\u0627 \u0648\u0645\u0639\u062A\u0645\u062F\u0629 \u0641\u064A \u0627\u0644\u0645\u062F\u064A\u0646\u0629",maxVisible:4});try{import("../components/WideAdsBanner.js").then(({mountWideAdsBanner:l})=>l("wide-ads-banner")).catch(()=>{})}catch{}Le()})}catch{}}function me(n,r=null,e=!1){const t=new Set,i=[],o=[];return[...n].sort((c,a)=>{const v=Number(c.createdAt)||Number(c.updatedAt)||0;return(Number(a.createdAt)||Number(a.updatedAt)||0)-v}).forEach(c=>{const a=c._key||c.id;t.has(a)||(t.add(a),ie(c)?i.push(c):o.push(c))}),[...e?ve(i):i,...o]}function he(n){const r=document.getElementById("categories-grid");!r||!n||(r.innerHTML=n.map(e=>{const t=e.slug||e._key||e.id||"",i=de(e),o=pe(e,{size:40});return`
      <a href="category.html?slug=${encodeURIComponent(t)}"
    ...
    `}).join(""))}const V="manzala_verified_showcase_v1";let A=null,w=[],I=0;const ge=[{id:"p_1788703900620_oae8ka",slug:"mtam-basl-wbaha-llmakwlat-albhrya",name:"\u0645\u0637\u0639\u0645 \u0628\u0627\u0633\u0644 \u0648\u0628\u0627\u0647\u0649 \u0644\u0644\u0645\u0623\u0643\u0648\u0644\u0627\u062A \u0627\u0644\u0628\u062D\u0631\u064A\u0629",area:"\u0627\u0644\u0645\u0637\u0631\u064A\u0629 \u062F\u0642\u0647\u0644\u064A\u0629",address:"\u0627\u0644\u0645\u0637\u0631\u064A\u0629 - \u0634 \u0627\u0644\u062B\u0648\u0631\u0629",phone:"01062944644",whatsapp:"01062944644",category:"\u0645\u0637\u0627\u0639\u0645 \u0648\u0623\u0633\u0645\u0627\u0643",cover:"https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"-P03LX9MledW_z7QfyHO",slug:"-P03LX9MledW_z7QfyHO",name:"\u0627\u0644\u062D\u0633\u0646 \u0644\u0635\u064A\u0627\u0646\u0629 \u0627\u0644\u0647\u0648\u0627\u062A\u0641 \u0627\u0644\u0645\u062D\u0645\u0648\u0644\u0629",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0634\u0627\u0631\u0639 \u0627\u0644\u0628\u062D\u0631",address:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0634\u0627\u0631\u0639 \u0627\u0644\u0628\u062D\u0631 \u0623\u0645\u0627\u0645 \u0627\u0644\u0628\u0646\u0643",phone:"01026046049",whatsapp:"01026046049",category:"\u0635\u064A\u0627\u0646\u0629 \u0648\u0645\u0648\u0628\u0627\u064A\u0644",cover:"https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"p_1788801925745_vuxmjs",slug:"mtbkh-eyma-llaakl-albyty",name:"\u0645\u0637\u0628\u062E \u0625\u064A\u0645\u0649 \u0644\u0644\u0623\u0643\u0644 \u0627\u0644\u0628\u064A\u062A\u064A",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0637\u0631\u064A\u0642 \u0627\u0644\u0645\u0646\u0635\u0648\u0631\u0629",address:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0637\u0631\u064A\u0642 \u0627\u0644\u0645\u0646\u0635\u0648\u0631\u0629 \u0627\u0644\u0631\u0626\u064A\u0633\u064A",phone:"01090123456",whatsapp:"01090123456",category:"\u0623\u0643\u0644 \u0628\u064A\u062A\u064A \u0648\u062D\u0644\u0648\u064A\u0627\u062A",cover:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1},{id:"-P0hEa0K6ZfAM65O27G9",slug:"kwafyr-mnh-asad",name:"\u0643\u0648\u0627\u0641\u064A\u0631 \u0645\u0646\u0647 \u0623\u0633\u0639\u062F",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u062D\u064A \u0627\u0644\u0633\u0644\u0627\u0645",address:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u062D\u064A \u0627\u0644\u0633\u0644\u0627\u0645",phone:"01099887766",whatsapp:"01099887766",category:"\u0628\u064A\u0648\u062A\u064A \u0648\u0643\u0648\u0627\u0641\u064A\u0631",cover:"https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=300&h=180&q=75",coverImageUrl:"https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1}];function Z(n=null){const r=document.getElementById("home-verified-cards-grid"),e=document.getElementById("home-verified-status-text");if(!r)return;let t=null;try{const c=localStorage.getItem(V);if(c){const a=JSON.parse(c);Array.isArray(a)&&a.length>0&&(t=a)}}catch{}if(Array.isArray(n)&&n.length>0){const c=n.filter(a=>a&&a.isVerified&&!te(a)).map(a=>({...a,id:a.id||a._key,slug:a.slug||a.id,name:a.name,area:a.area||"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",address:a.address||"",phone:a.phone||"",whatsapp:a.whatsapp||"",logoUrl:a.logoUrl||"",coverImageUrl:a.coverImageUrl||a.gallery&&a.gallery[0]||"",category:a.categoryName||a.customCategory||a.categoryId||"\u0646\u0634\u0627\u0637 \u062A\u062C\u0627\u0631\u064A",cover:a.coverImageUrl||a.logoUrl||a.gallery&&a.gallery[0]||"/assets/images/og-whatsapp.jpg",isSponsored:!!(a.isSponsored&&(!a.sponsoredUntil||a.sponsoredUntil>Date.now()))}));if(c.length>0){w=c;try{localStorage.setItem(V,JSON.stringify(c))}catch{}}}else w.length||(t&&t.length>0?w=t:w=[...ge]);if(typeof window<"u"&&Array.isArray(w)){window._placesRegistry=window._placesRegistry||new Map;for(const c of w){if(!c)continue;const a=String(c.slug||c.id||"").toLowerCase().trim();a&&(window._placesRegistry.set(a,c),c.slug&&window._placesRegistry.set(String(c.slug).toLowerCase().trim(),c),c.id&&window._placesRegistry.set(String(c.id).toLowerCase().trim(),c))}}const i=["\u{1F947} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #1","\u{1F948} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #2","\u{1F949} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #3","\u{1F396}\uFE0F \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #4"];function o(c){r.innerHTML=c.map((a,v)=>{const l=a.slug||a.id||"";return`
      <article class="fair-place-card" data-card-index="${v}"
        ...
        <div class="fair-place-card__cover">
          <img src="${s(a.coverImageUrl||a.cover)}" alt="${s(a.name)}" loading="lazy" onerror="this.src='/assets/images/og-whatsapp.jpg'">
          <div class="fair-place-card__badges">
            ${a.isSponsored?'<span class="fair-badge-sponsored">\u2B50 \u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632</span>':""}
            <span class="fair-badge-verified">\u2713 \u0645\u0648\u062B\u0642 \u0631\u0633\u0645\u064A\u0627\u064B</span>
          </div>
        </div>
        <div class="fair-place-card__body">
          <h3 class="fair-place-card__title" title="${s(a.name)}">${_(a.name)}</h3>
          <div class="fair-place-card__meta">
            <span>\u{1F4CD} ${_(a.area)}</span>
            <span>\u{1F3F7}\uFE0F ${_(a.category)}</span>
          </div>
    ...
    `}).join("")}const d=w.length,h=[];for(let c=0;c<Math.min(4,d);c++)h.push(w[(I+c)%d]);if(o(h),A&&(clearInterval(A),A=null),d>=2){let c=1;A=setInterval(()=>{r.querySelectorAll(".fair-place-card").forEach(v=>v.classList.add("anim-swap")),setTimeout(()=>{I=(I+1)%w.length,c++;const v=[],l=w.length;for(let g=0;g<Math.min(4,l);g++)v.push(w[(I+g)%l]);o(v),e&&(e.innerHTML=`\u{1F7E2} <b>\u062A\u0645 \u062A\u062F\u0648\u064A\u0631 \u0627\u0644\u0635\u062F\u0627\u0631\u0629 \u062A\u0644\u0642\u0627\u0626\u064A\u0627\u064B (${c}):</b> \u062A\u062A\u063A\u064A\u0631 \u0627\u0644\u0645\u0631\u0627\u0643\u0632 \u062F\u0648\u0631\u064A\u0627\u064B \u0644\u0636\u0645\u0627\u0646 \u062A\u0643\u0627\u0641\u0624 \u0646\u0633\u0628 \u0627\u0644\u0645\u0634\u0627\u0647\u062F\u0629 \u0644\u0643\u0627\u0641\u0629 \u0627\u0644\u0623\u0645\u0627\u0643\u0646 \u0627\u0644\u0645\u0648\u062B\u0642\u0629!`)},300)},4500)}}function ve(n){const r=[...n];for(let e=r.length-1;e>0;e--){const t=Math.floor(Math.random()*(e+1));[r[e],r[t]]=[r[t],r[e]]}return r}function ue(n){const r=document.getElementById("latest-places-grid");if(r){if(!n||!n.length){r.innerHTML=`
    ...
    `;return}r.innerHTML=n.map(e=>X(e)).join("")}}function fe(n){const r=document.getElementById("offers-scroll"),e=document.getElementById("offers-section");if(r){if(!n||!n.length){e?.remove();return}r.innerHTML=n.map(t=>{const i=t.discountPercent||re(t.oldPrice,t.newPrice),o=oe(t.endDate);return`
      <article class="offer-card"
               data-place-slug="${s(t.placeSlug||"")}"
               data-name="${s(t.placeName||t.title||"")}"
               data-cover="${s(t.imageUrl||"")}"
               onclick="window.__openPlaceCard ? window.__openPlaceCard(this, '${s(t.placeSlug||"")}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(t.placeSlug||"")}')"
               ontouchstart="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${s(t.placeSlug||"")}', this)"
               onpointerdown="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${s(t.placeSlug||"")}', this)"
               onmouseenter="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${s(t.placeSlug||"")}', this)"
               style="cursor:pointer">
        <div class="offer-card__image">
          ${t.imageUrl?`<img src="${s(t.imageUrl)}" alt="${s(t.title)}" loading="lazy" />`:'<div style="width:100%;height:100%;background:var(--primary-alpha);display:flex;align-items:center;justify-content:center;font-size:2rem">\u{1F3F7}\uFE0F</div>'}
        ...
        <div class="offer-card__body">
          ...
          <div class="offer-card__price">
            <span class="offer-card__price-new">${F(t.newPrice)}</span>
            ${t.oldPrice?`<span class="offer-card__price-old">${F(t.oldPrice)}</span>`:""}
          </div>
          `:""}
    ...
    `}).join("")}}function be(n){const r=document.getElementById("delivery-grid"),e=document.getElementById("delivery-section");if(!r)return;if(!n||!n.length){e?.remove();return}const t={motorcycle:"\u{1F3CD}\uFE0F",tuktuk:"\u{1F6FA}",car:"\u{1F697}",taxi:"\u{1F695}",bus:"\u{1F68C}",pickup_quarter:"\u{1F6FB}",pickup_half:"\u{1F69A}",truck_heavy:"\u{1F69B}",cart_donkey:"\u{1FACF}",cart_horse:"\u{1F40E}",loader:"\u{1F69C}",tractor_trailer:"\u{1F69C}"};r.innerHTML=n.slice(0,6).map(i=>{const o=i.slug||i._key||i.id||"";return`
    <a href="/place.html?slug=${encodeURIComponent(o)}" class="delivery-card"
      ...
      <div class="delivery-card__icon">${t[i.deliveryType]||"\u{1F680}"}</div>
  ...
  `}).join("")}function ye(n){const r=document.getElementById("ads-container");if(!r||!n||!n.length)return;const e=n.filter(t=>t&&(t.imageUrl||t.image_url));if(!e.length){r.innerHTML="";return}r.innerHTML=e.map(t=>{let i=(t.link||"#").trim();i.startsWith("place.html")&&(i="/"+i);const o=t.id||t._id||"",d=t.title||"\u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632",h=t.imageUrl||t.image_url||"";return`
    ...
    `}).join(""),r.querySelectorAll(".ad-banner").forEach(t=>{t.addEventListener("click",()=>{const i=t.getAttribute("data-ad-id");if(i&&R)try{fetch(`${R}/api/ads/track-click`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:i}),keepalive:!0}).catch(()=>{})}catch{}})})}class _e{constructor(){this.ctx=null,this.lastTickTime=0}init(){if(!this.ctx&&typeof window<"u"){const r=window.AudioContext||window.webkitAudioContext;r&&(this.ctx=new r)}}playTick(r=550){try{if(!this.ctx||this.ctx.state!=="running")return;const e=this.ctx.currentTime;if(e-this.lastTickTime<.038)return;this.lastTickTime=e;const t=this.ctx.createOscillator(),i=this.ctx.createGain();t.type="sine",t.frequency.setValueAtTime(r,e),t.frequency.exponentialRampToValueAtTime(r*1.35,e+.018),i.gain.setValueAtTime(.04,e),i.gain.exponentialRampToValueAtTime(1e-4,e+.022),t.connect(i),i.connect(this.ctx.destination),t.start(e),t.stop(e+.028)}catch{}}playDoneChime(){try{if(!this.ctx||this.ctx.state!=="running")return;const r=this.ctx.currentTime;[1046.5,1318.51,1567.98,2093].forEach((t,i)=>{const o=this.ctx.createOscillator(),d=this.ctx.createGain();o.type="triangle",o.frequency.setValueAtTime(t,r+i*.055),d.gain.setValueAtTime(.06,r+i*.055),d.gain.exponentialRampToValueAtTime(1e-4,r+i*.055+.32),o.connect(d),d.connect(this.ctx.destination),o.start(r+i*.055),o.stop(r+i*.055+.35)})}catch{}}}const $=new _e;function we(n,r){const e=document.getElementById("stats-bar");if(!e)return;const t=5e4,i=Math.max(15e3,Number(n)||0),o=12e3,d=Math.max(124,Number(r)||0),h=55;e.innerHTML=`
  ...
  `,xe(e)}function xe(n){let r=!1,e=!1;function t(){if(e)return;e=!0;const i=n.querySelectorAll(".stats-bar__value[data-target]");if(!i.length){e=!1;return}const o=1800,d=performance.now();i.forEach(a=>{a.classList.remove("stats-done"),a.classList.add("stats-counting")});let h=-1;function c(a){const v=a-d,l=Math.min(1,v/o),g=1-Math.pow(1-l,3);i.forEach(u=>{const y=parseInt(u.getAttribute("data-target"),10)||0,p=u.getAttribute("data-prefix")||"",f=u.getAttribute("data-suffix")||"",b=Math.floor(g*y);if(u.textContent=`${p}${b.toLocaleString("en-US")}${f}`,b!==h){h=b;const M=420+b/Math.max(1,y)*430;$.playTick(M)}}),l<1?requestAnimationFrame(c):(i.forEach(u=>{const y=parseInt(u.getAttribute("data-target"),10)||0,p=u.getAttribute("data-prefix")||"",f=u.getAttribute("data-suffix")||"";u.textContent=`${p}${y.toLocaleString("en-US")}${f}`,u.classList.remove("stats-counting"),u.classList.add("stats-done")}),$.playDoneChime(),e=!1)}requestAnimationFrame(c)}if("IntersectionObserver"in window){const i=new IntersectionObserver(o=>{o.forEach(d=>{d.isIntersecting&&!r&&(r=!0,t(),i.disconnect())})},{threshold:.15});i.observe(n)}else t();n.querySelectorAll(".stats-interactive-item").forEach(i=>{i.addEventListener("click",()=>{$.init(),$.ctx&&$.ctx.state==="suspended"&&$.ctx.resume().catch(()=>{}),t()})})}function Ce(n){const r=document.getElementById("hero-search-glow-wrap"),e=document.getElementById("hero-search-input"),t=document.getElementById("hero-search-btn"),i=document.getElementById("hero-search-clear"),o=document.getElementById("hero-live-dropdown");if(!e)return;o&&!document.getElementById("hero-live-list")&&(o.innerHTML=`
    ...
    `);const d=document.getElementById("hero-live-list"),h=document.getElementById("hero-live-count"),c=document.getElementById("hero-live-all-btn"),a=document.getElementById("hero-quick-cats");a&&n&&(a.innerHTML=n.slice(0,10).map(p=>{const f=p.slug||p._key||p.id||"",b=le(f||p.name,18);return`
        <a href="category.html?slug=${encodeURIComponent(f)}" class="hero__quick-cat">
          ${b||p.icon||"\u{1F3EA}"} ${_(p.name)}
        </a>
      `}).join(""));function v(){const p=e.value.trim();p&&(window.location.href=`search.html?q=${encodeURIComponent(p)}`)}t?.addEventListener("click",p=>{p.stopPropagation(),v()}),e.addEventListener("keydown",p=>{p.key==="Enter"?v():p.key==="Escape"&&o?.classList.remove("visible")}),i?.addEventListener("click",p=>{p.stopPropagation(),e.value="",i.classList.remove("visible"),o?.classList.remove("visible"),d&&(d.innerHTML=""),e.focus()});function l(){!o||!d||(h&&(h.textContent="\u0645\u0642\u062A\u0631\u062D\u0627\u062A"),d.innerHTML=`
    ...
    `,d.querySelectorAll(".hero-live-suggestion-chip").forEach(p=>{p.addEventListener("click",f=>{f.preventDefault(),f.stopPropagation();const b=p.getAttribute("data-q")||"";e.value=b,e.dispatchEvent(new Event("input",{bubbles:!0})),e.focus()})}),o.classList.add("visible"))}e.addEventListener("focus",()=>{e.value.trim().length>=1&&d?.children.length>0?o?.classList.add("visible"):e.value.trim()||l()}),e.addEventListener("click",()=>{e.value.trim()||l()});let g=null,u=0;e.addEventListener("input",()=>{const p=e.value.trim();if(i?.classList.toggle("visible",p.length>0),c&&(c.href=`search.html?q=${encodeURIComponent(p)}`),!p){o?.classList.remove("visible"),d&&(d.innerHTML="");return}clearTimeout(g),g=setTimeout(async()=>{const f=++u;try{let M=function(C,m){if(!C)return"";if(!m)return _(C);const k=m.trim().split(/\s+/).filter(Boolean);if(!k.length)return _(C);const S=k.map(x=>x.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")),E=new RegExp(`(${S.join("|")})`,"gi");return _(C).replace(E,'<span class="search-highlight">$1</span>')};const b=await ce(p,{limit:6});if(f!==u||!o||!d)return;if(!b||b.length===0){h&&(h.textContent="0"),d.innerHTML=`
          ...
          `,o.classList.add("visible");return}h&&(h.textContent=String(b.length)),d.innerHTML=b.map(C=>{const m=C.raw||C,k=m.name||"\u0645\u0643\u0627\u0646 \u0628\u0627\u0644\u062F\u0644\u064A\u0644",S=m.categoryName||C.category||"",E=m.area||m.address||"\u0645\u062F\u064A\u0646\u0629 \u0627\u0644\u0645\u0646\u0632\u0644\u0629",x=m.slug||m.id||"",B=m.photoURL||m.logo||m.coverURL||m.coverImageUrl||m.logoUrl||"",W=m.isVerified||!1,q=m.isOpen!==void 0?m.isOpen:!0,z=(k.trim()[0]||"\u0645").toUpperCase(),T=(m.phone||"").trim(),U=(m.whatsapp||m.phone||"").trim(),L=U?U.replace(/[^0-9]/g,""):"",P=L?L.startsWith("2")?L:L.startsWith("0")?"2"+L:"20"+L:"";let H="";if((T||P)&&(H=`
              <div class="hero-live-actions" onclick="event.stopPropagation()">
                ${T?`
                  <a href="tel:${s(T)}" class="hero-live-action-btn hero-live-action-btn--call" title="\u0627\u062A\u0635\u0627\u0644 \u0647\u0627\u062A\u0641\u064A \u0645\u0628\u0627\u0634\u0631" onclick="event.stopPropagation()">
                ...
                `:""}
                ${P?`
                  <a href="https://wa.me/${s(P)}" target="_blank" rel="noopener" class="hero-live-action-btn hero-live-action-btn--wa" title="\u0645\u062D\u0627\u062F\u062B\u0629 \u0648\u0627\u062A\u0633\u0627\u0628 \u0641\u0648\u0631\u064A\u0629" onclick="event.stopPropagation()">
            ...
            <a href="/place.html?slug=${encodeURIComponent(x)}" class="hero-live-dropdown__item" role="option"
              ...
              <div class="hero-live-avatar">
                ${B?`<img src="${s(B)}" alt="${s(k)}" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML='<div class=\\'hero-live-avatar-fallback\\'>${z}</div>'"/>`:`<div class="hero-live-avatar-fallback">${z}</div>`}
              </div>
              <div class="hero-live-content">
                <div class="hero-live-title-row">
                  <span class="hero-live-name">${M(k,p)}</span>
  ...
  `}function _(n){return n?String(n).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}function s(n){return n?String(n).replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function Le(){try{let i=function(){try{localStorage.setItem("__has_seen_welcome_video_v1","true")}catch{}e&&e.pause(),r.style.opacity="0";const o=r.querySelector(".first-visit-video-card");o&&(o.style.transform="scale(0.92)"),setTimeout(()=>{r&&r.parentNode&&r.parentNode.removeChild(r)},350)};return}catch(n){console.warn("[Welcome Video] error:",n)}}
