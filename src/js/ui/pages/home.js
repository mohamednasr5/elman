import{getCategories as R,getPublishedPlaces as q,getActiveOffers as U,getAds as V}from"../../core/db.js";import{WORKER_URL as A}from"../../core/firebase.js";import"../../core/state.js";import{renderPlaceCard as D,renderPlaceCardSkeleton as Z}from"../components/PlaceCard.js";import{isAtmPlace as O}from"../../utils/atm.js";import{mountSponsoredShowcase as N,isPlaceSponsored as W}from"../components/SponsoredShowcase.js";import{formatPrice as T,calcDiscount as Q,arabicMatch as I}from"../../utils/arabic.js";import{daysUntil as G}from"../../utils/date.js";import{getCurrentUser as J}from"../../core/auth.js";import{openManzalaVoiceAssistantModal as K}from"../../services/voice.service.js";import{mountLivePulseSection as Y}from"../components/LivePulseSection.js?v=c1cf1c7c";import{mountAroundMeRadar as X}from"../components/AroundMeRadar.js";import{executeFastSearch as ee}from"../../services/search-engine.service.js";import{getCategorySvg as ae}from"../../utils/professions-data.js";import{getCategoryVisualMeta as te,renderCategoryCardIcon as ie}from"../../utils/category-visual.js";import{renderWhoIsAvailableNow as re}from"../components/WhoIsAvailableNow.js";const Ve={pharmacy:{emoji:"\u{1F48A}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},supermarket:{emoji:"\u{1F6D2}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},paint:{emoji:"\u{1F3A8}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},herbs:{emoji:"\u{1F33F}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},doctor:{emoji:"\u{1F468}\u200D\u2695\uFE0F",color:"rgba(41,128,185,0.1)",border:"#2980B9"},plumbing:{emoji:"\u{1F527}",color:"rgba(52,73,94,0.1)",border:"#52596E"},plumber:{emoji:"\u{1FAA0}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpenter:{emoji:"\u{1FA9A}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},tiler:{emoji:"\u{1F9F1}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},painter:{emoji:"\u{1F58C}\uFE0F",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},electrician:{emoji:"\u26A1",color:"rgba(243,156,18,0.1)",border:"#F39C12"},"ac-technician":{emoji:"\u2744\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},blacksmith:{emoji:"\u{1F6E0}\uFE0F",color:"rgba(52,73,94,0.1)",border:"#52596E"},alumital:{emoji:"\u{1FA9F}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},mechanic:{emoji:"\u{1F529}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},upholsterer:{emoji:"\u{1F6CB}\uFE0F",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},feed:{emoji:"\u{1F33E}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},poultry:{emoji:"\u{1F357}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},bakery:{emoji:"\u{1F35E}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},vegetables:{emoji:"\u{1F96C}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},antiques:{emoji:"\u{1F3FA}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},electronics:{emoji:"\u{1F4FA}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpet:{emoji:"\u{1F9F6}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},mattress:{emoji:"\u{1F6CF}\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},china:{emoji:"\u{1F37D}\uFE0F",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},electrical:{emoji:"\u{1F4A1}",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},roastery:{emoji:"\u{1F95C}",color:"rgba(101,67,33,0.1)",border:"#654321"},phones:{emoji:"\u{1F4F1}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},grocery:{emoji:"\u{1F3EA}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},hypermarket:{emoji:"\u{1F3EC}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"},delivery:{emoji:"\u{1F680}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"}},De={emoji:"\u{1F3EA}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"};export async function renderHomePage(o,{user:t}={}){o.innerHTML=ye(),P();try{const[e,a,i,r]=await Promise.all([R(),q({limit:100}),U(8),V("homepage")]),s=J()||t,m=a||[];if(typeof window<"u"&&Array.isArray(m)){window._placesRegistry=window._placesRegistry||new Map;for(const d of m){if(!d)continue;const p=String(d.slug||d.id||d._key||"").toLowerCase().trim();p&&(window._placesRegistry.set(p,d),d.id&&window._placesRegistry.set(String(d.id).toLowerCase().trim(),d),d.slug&&window._placesRegistry.set(String(d.slug).toLowerCase().trim(),d))}}const l=document.getElementById("home-oncall-craftsmen-container");l&&re(l),Y("home-live-pulse-container"),X("home-around-me-container"),N("home-sponsored-container",m,{title:"\u0623\u0645\u0627\u0643\u0646 \u0648\u0625\u0639\u0644\u0627\u0646\u0627\u062A \u0645\u0645\u064A\u0632\u0629 \u0641\u064A \u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",subtitle:"\u0623\u0646\u0634\u0637\u0629 \u062A\u062C\u0627\u0631\u064A\u0629 \u0648\u062E\u062F\u0645\u0627\u062A \u0645\u0648\u0635\u0649 \u0628\u0647\u0627 \u0648\u0645\u0639\u062A\u0645\u062F\u0629 \u0641\u064A \u0627\u0644\u0645\u062F\u064A\u0646\u0629",maxVisible:4}),ne(e||[]),P(m);const n=oe(m,s?.uid);le(n.slice(0,8)),de(i||[]),pe(m.filter(d=>d.categoryId?.includes("delivery")||d.deliveryType)),me(r||[]);try{import("../components/WideAdsBanner.js").then(({mountWideAdsBanner:d})=>d("wide-ads-banner")).catch(()=>{})}catch{}fe(e||[]),ue(),be(),ve(m.length||0,e?.length||31),_e()}catch{}}function oe(o,t=null,e=!1){const a=new Set,i=[],r=[];return[...o].sort((l,n)=>{const d=Number(l.createdAt)||Number(l.updatedAt)||0;return(Number(n.createdAt)||Number(n.updatedAt)||0)-d}).forEach(l=>{const n=l._key||l.id;a.has(n)||(a.add(n),W(l)?i.push(l):r.push(l))}),[...e?ce(i):i,...r]}function ne(o){const t=document.getElementById("categories-grid");!t||!o||(t.innerHTML=o.map(e=>{const a=e.slug||e._key||e.id||"",i=te(e),r=ie(e,{size:40});return`
    ...
    `}).join(""))}const j="manzala_verified_showcase_v1";let C=null,_=[],k=0;const se=[{id:"p_1788703900620_oae8ka",slug:"mtam-basl-wbaha-llmakwlat-albhrya",name:"\u0645\u0637\u0639\u0645 \u0628\u0627\u0633\u0644 \u0648\u0628\u0627\u0647\u0649 \u0644\u0644\u0645\u0623\u0643\u0648\u0644\u0627\u062A \u0627\u0644\u0628\u062D\u0631\u064A\u0629",area:"\u0627\u0644\u0645\u0637\u0631\u064A\u0629 \u062F\u0642\u0647\u0644\u064A\u0629",category:"\u0645\u0637\u0627\u0639\u0645 \u0648\u0623\u0633\u0645\u0627\u0643",cover:"https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"-P03LX9MledW_z7QfyHO",slug:"-P03LX9MledW_z7QfyHO",name:"\u0627\u0644\u062D\u0633\u0646 \u0644\u0635\u064A\u0627\u0646\u0629 \u0627\u0644\u0647\u0648\u0627\u062A\u0641 \u0627\u0644\u0645\u062D\u0645\u0648\u0644\u0629",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0634\u0627\u0631\u0639 \u0627\u0644\u0628\u062D\u0631",category:"\u0635\u064A\u0627\u0646\u0629 \u0648\u0645\u0648\u0628\u0627\u064A\u0644",cover:"https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"p_1788801925745_vuxmjs",slug:"mtbkh-eyma-llaakl-albyty",name:"\u0645\u0637\u0628\u062E \u0625\u064A\u0645\u0649 \u0644\u0644\u0623\u0643\u0644 \u0627\u0644\u0628\u064A\u062A\u064A",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0637\u0631\u064A\u0642 \u0627\u0644\u0645\u0646\u0635\u0648\u0631\u0629",category:"\u0623\u0643\u0644 \u0628\u064A\u062A\u064A \u0648\u062D\u0644\u0648\u064A\u0627\u062A",cover:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1},{id:"-P0hEa0K6ZfAM65O27G9",slug:"kwafyr-mnh-asad",name:"\u0643\u0648\u0627\u0641\u064A\u0631 \u0645\u0646\u0647 \u0623\u0633\u0639\u062F",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u062D\u064A \u0627\u0644\u0633\u0644\u0627\u0645",category:"\u0628\u064A\u0648\u062A\u064A \u0648\u0643\u0648\u0627\u0641\u064A\u0631",cover:"https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1}];function P(o=null){const t=document.getElementById("home-verified-cards-grid"),e=document.getElementById("home-verified-status-text");if(!t)return;let a=null;try{const l=localStorage.getItem(j);if(l){const n=JSON.parse(l);Array.isArray(n)&&n.length>0&&(a=n)}}catch{}if(Array.isArray(o)&&o.length>0){const l=o.filter(n=>n&&n.isVerified&&!O(n)).map(n=>({id:n.id||n._key,slug:n.slug||n.id,name:n.name,area:n.area||"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",category:n.categoryName||n.customCategory||n.categoryId||"\u0646\u0634\u0627\u0637 \u062A\u062C\u0627\u0631\u064A",cover:n.coverImageUrl||n.logoUrl||n.gallery&&n.gallery[0]||"/assets/images/og-whatsapp.jpg",isSponsored:!!(n.isSponsored&&(!n.sponsoredUntil||n.sponsoredUntil>Date.now()))}));if(l.length>0){_=l;try{localStorage.setItem(j,JSON.stringify(l))}catch{}}}else _.length||(a&&a.length>0?_=a:_=[...se]);const i=["\u{1F947} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #1","\u{1F948} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #2","\u{1F949} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #3","\u{1F396}\uFE0F \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #4"];function r(l){t.innerHTML=l.map((n,d)=>{const p=n.slug||n.id||"";return`
      <article class="fair-place-card" data-card-index="${d}"
               onclick="window.__openPlaceCard ? window.__openPlaceCard(this, '${g(p)}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(p)}')"
               onpointerdown="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(p)}')"
               onmouseenter="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(p)}')"
               style="cursor:pointer">
        <span class="fair-place-card__rank">${i[d]||`\u{1F396}\uFE0F \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #${d+1}`}</span>
        <div class="fair-place-card__cover">
          <img src="${g(n.cover)}" alt="${g(n.name)}" loading="lazy" onerror="this.src='/assets/images/og-whatsapp.jpg'">
          <div class="fair-place-card__badges">
            ${n.isSponsored?'<span class="fair-badge-sponsored">\u2B50 \u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632</span>':""}
            <span class="fair-badge-verified">\u2713 \u0645\u0648\u062B\u0642 \u0631\u0633\u0645\u064A\u0627\u064B</span>
          </div>
        </div>
        <div class="fair-place-card__body">
          <h3 class="fair-place-card__title" title="${g(n.name)}">${y(n.name)}</h3>
          <div class="fair-place-card__meta">
            <span>\u{1F4CD} ${y(n.area)}</span>
            <span>\u{1F3F7}\uFE0F ${y(n.category)}</span>
          </div>
          <a href="/place.html?slug=${encodeURIComponent(p)}" class="fair-place-card__link" onclick="event.preventDefault(); window.__openPlaceCard ? window.__openPlaceCard(this, '${g(p)}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(p)}')">\u0639\u0631\u0636 \u0628\u0637\u0627\u0642\u0629 \u0627\u0644\u0645\u0643\u0627\u0646 \u2197</a>
        </div>
      </article>
    `}).join("")}const s=_.length,m=[];for(let l=0;l<Math.min(4,s);l++)m.push(_[(k+l)%s]);if(r(m),C&&(clearInterval(C),C=null),s>=2){let l=1;C=setInterval(()=>{t.querySelectorAll(".fair-place-card").forEach(d=>d.classList.add("anim-swap")),setTimeout(()=>{k=(k+1)%_.length,l++;const d=[],p=_.length;for(let v=0;v<Math.min(4,p);v++)d.push(_[(k+v)%p]);r(d),e&&(e.innerHTML=`\u{1F7E2} <b>\u062A\u0645 \u062A\u062F\u0648\u064A\u0631 \u0627\u0644\u0635\u062F\u0627\u0631\u0629 \u062A\u0644\u0642\u0627\u0626\u064A\u0627\u064B (${l}):</b> \u062A\u062A\u063A\u064A\u0631 \u0627\u0644\u0645\u0631\u0627\u0643\u0632 \u062F\u0648\u0631\u064A\u0627\u064B \u0644\u0636\u0645\u0627\u0646 \u062A\u0643\u0627\u0641\u0624 \u0646\u0633\u0628 \u0627\u0644\u0645\u0634\u0627\u0647\u062F\u0629 \u0644\u0643\u0627\u0641\u0629 \u0627\u0644\u0623\u0645\u0627\u0643\u0646 \u0627\u0644\u0645\u0648\u062B\u0642\u0629!`)},300)},4500)}}function ce(o){const t=[...o];for(let e=t.length-1;e>0;e--){const a=Math.floor(Math.random()*(e+1));[t[e],t[a]]=[t[a],t[e]]}return t}function le(o){const t=document.getElementById("latest-places-grid");if(t){if(!o||!o.length){t.innerHTML=`
    ...
    `;return}t.innerHTML=o.map(e=>D(e)).join("")}}function de(o){const t=document.getElementById("offers-scroll"),e=document.getElementById("offers-section");if(t){if(!o||!o.length){e?.remove();return}t.innerHTML=o.map(a=>{const i=a.discountPercent||Q(a.oldPrice,a.newPrice),r=G(a.endDate);return`
    ...
    `}).join("")}}function pe(o){const t=document.getElementById("delivery-grid"),e=document.getElementById("delivery-section");if(!t)return;if(!o||!o.length){e?.remove();return}const a={motorcycle:"\u{1F3CD}\uFE0F",tuktuk:"\u{1F6FA}",car:"\u{1F697}",taxi:"\u{1F695}",bus:"\u{1F68C}",pickup_quarter:"\u{1F6FB}",pickup_half:"\u{1F69A}",truck_heavy:"\u{1F69B}",cart_donkey:"\u{1FACF}",cart_horse:"\u{1F40E}",loader:"\u{1F69C}",tractor_trailer:"\u{1F69C}"};t.innerHTML=o.slice(0,6).map(i=>{const r=i.slug||i._key||i.id||"";return`
  ...
  `}).join("")}function me(o){const t=document.getElementById("ads-container");if(!t||!o||!o.length)return;const e=o.filter(a=>a&&(a.imageUrl||a.image_url));if(!e.length){t.innerHTML="";return}t.innerHTML=e.map(a=>{let i=(a.link||"#").trim();i.startsWith("place.html")&&(i="/"+i);const r=a.id||a._id||"",s=a.title||"\u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632",m=a.imageUrl||a.image_url||"";return`
      <a href="${g(i)}" class="ad-banner" target="_blank" rel="noopener noreferrer sponsored" aria-label="${g(s)}" data-ad-id="${g(r)}">
        ...
        <img src="${g(m)}" alt="${g(s)}" loading="lazy" decoding="async" />
      </a>
    `}).join(""),t.querySelectorAll(".ad-banner").forEach(a=>{a.addEventListener("click",()=>{const i=a.getAttribute("data-ad-id");if(i&&A)try{fetch(`${A}/api/ads/track-click`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:i}),keepalive:!0}).catch(()=>{})}catch{}})})}class ge{constructor(){this.ctx=null,this.lastTickTime=0}init(){if(!this.ctx&&typeof window<"u"){const t=window.AudioContext||window.webkitAudioContext;t&&(this.ctx=new t)}}playTick(t=550){try{if(!this.ctx||this.ctx.state!=="running")return;const e=this.ctx.currentTime;if(e-this.lastTickTime<.038)return;this.lastTickTime=e;const a=this.ctx.createOscillator(),i=this.ctx.createGain();a.type="sine",a.frequency.setValueAtTime(t,e),a.frequency.exponentialRampToValueAtTime(t*1.35,e+.018),i.gain.setValueAtTime(.04,e),i.gain.exponentialRampToValueAtTime(1e-4,e+.022),a.connect(i),i.connect(this.ctx.destination),a.start(e),a.stop(e+.028)}catch{}}playDoneChime(){try{if(!this.ctx||this.ctx.state!=="running")return;const t=this.ctx.currentTime;[1046.5,1318.51,1567.98,2093].forEach((a,i)=>{const r=this.ctx.createOscillator(),s=this.ctx.createGain();r.type="triangle",r.frequency.setValueAtTime(a,t+i*.055),s.gain.setValueAtTime(.06,t+i*.055),s.gain.exponentialRampToValueAtTime(1e-4,t+i*.055+.32),r.connect(s),s.connect(this.ctx.destination),r.start(t+i*.055),r.stop(t+i*.055+.35)})}catch{}}}const w=new ge;function ve(o,t){const e=document.getElementById("stats-bar");if(!e)return;const a=Math.max(191,Number(o)||0),i=Math.max(94,Number(t)||0),r=55;e.innerHTML=`
  ...
  `,he(e)}function he(o){let t=!1,e=!1;function a(){if(e)return;e=!0;const i=o.querySelectorAll(".stats-bar__value[data-target]");if(!i.length){e=!1;return}const r=1800,s=performance.now();i.forEach(n=>{n.classList.remove("stats-done"),n.classList.add("stats-counting")});let m=-1;function l(n){const d=n-s,p=Math.min(1,d/r),v=1-Math.pow(1-p,3);i.forEach(h=>{const c=parseInt(h.getAttribute("data-target"),10)||0,b=h.getAttribute("data-prefix")||"",f=h.getAttribute("data-suffix")||"",x=Math.floor(v*c);if(h.textContent=`${b}${x.toLocaleString("en-US")}${f}`,x!==m){m=x;const u=420+x/Math.max(1,c)*430;w.playTick(u)}}),p<1?requestAnimationFrame(l):(i.forEach(h=>{const c=parseInt(h.getAttribute("data-target"),10)||0,b=h.getAttribute("data-prefix")||"",f=h.getAttribute("data-suffix")||"";h.textContent=`${b}${c.toLocaleString("en-US")}${f}`,h.classList.remove("stats-counting"),h.classList.add("stats-done")}),w.playDoneChime(),e=!1)}requestAnimationFrame(l)}if("IntersectionObserver"in window){const i=new IntersectionObserver(r=>{r.forEach(s=>{s.isIntersecting&&!t&&(t=!0,a(),i.disconnect())})},{threshold:.15});i.observe(o)}else a();o.querySelectorAll(".stats-interactive-item").forEach(i=>{i.addEventListener("click",()=>{w.init(),w.ctx&&w.ctx.state==="suspended"&&w.ctx.resume().catch(()=>{}),a()})})}function fe(o){const t=document.getElementById("hero-search-glow-wrap"),e=document.getElementById("hero-search-input"),a=document.getElementById("hero-search-btn"),i=document.getElementById("hero-search-clear"),r=document.getElementById("hero-live-dropdown"),s=document.getElementById("hero-live-list"),m=document.getElementById("hero-live-count"),l=document.getElementById("hero-live-all-btn");if(!e)return;const n=document.getElementById("hero-quick-cats");n&&o&&(n.innerHTML=o.slice(0,10).map(c=>{const b=c.slug||c._key||c.id||"",f=ae(b||c.name,18);return`
        <a href="category.html?slug=${encodeURIComponent(b)}" class="hero__quick-cat">
          ${f||c.icon||"\u{1F3EA}"} ${y(c.name)}
        </a>
      `}).join(""));function d(){const c=e.value.trim();c&&(window.location.href=`search.html?q=${encodeURIComponent(c)}`)}a?.addEventListener("click",c=>{c.stopPropagation(),d()}),e.addEventListener("keydown",c=>{c.key==="Enter"?d():c.key==="Escape"&&r?.classList.remove("visible")}),i?.addEventListener("click",c=>{c.stopPropagation(),e.value="",i.classList.remove("visible"),r?.classList.remove("visible"),s&&(s.innerHTML=""),e.focus()}),e.addEventListener("focus",()=>{e.value.trim().length>=1&&s?.children.length>0&&r?.classList.add("visible")});let p=null,v=0;e.addEventListener("input",()=>{const c=e.value.trim();if(i?.classList.toggle("visible",c.length>0),l&&(l.href=`search.html?q=${encodeURIComponent(c)}`),!c){r?.classList.remove("visible"),s&&(s.innerHTML="");return}clearTimeout(p),p=setTimeout(async()=>{const b=++v;try{const f=await ee(c,{limit:6});if(b!==v||!r||!s)return;if(!f||f.length===0){m&&(m.textContent="0"),s.innerHTML=`
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--space-4);flex-wrap:wrap;gap:12px">
          <div>
            <h2 class="section-title" style="margin-bottom:2px">
              <span>\u{1F5FA}\uFE0F</span> \u0627\u0633\u062A\u0643\u0634\u0641 \u062D\u0633\u0628 \u0627\u0644\u0645\u062F\u064A\u0646\u0629 \u0648\u0627\u0644\u0642\u0631\u064A\u0629 (${o.length})
            </h2>
  ...
  `}function y(o){return o?String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}function g(o){return o?String(o).replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function _e(){try{let i=function(){try{localStorage.setItem("__has_seen_welcome_video_v1","true")}catch{}e&&e.pause(),t.style.opacity="0";const r=t.querySelector(".first-visit-video-card");r&&(r.style.transform="scale(0.92)"),setTimeout(()=>{t&&t.parentNode&&t.parentNode.removeChild(t)},350)};return}catch(o){console.warn("[Welcome Video] error:",o)}}
