import{getCategories as de,getPublishedPlaces as Q,getActiveOffers as J,getAds as pe,getCached as w,FALLBACK_CATEGORIES as R}from"../../core/db.js";import{WORKER_URL as K}from"../../core/firebase.js";import"../../core/state.js";import{renderPlaceCard as me,renderPlaceCardSkeleton as he}from"../components/PlaceCard.js";import{isAtmPlace as ge}from"../../utils/atm.js";import{mountSponsoredShowcase as ve,isPlaceSponsored as ue}from"../components/SponsoredShowcase.js";import{formatPrice as Y,calcDiscount as fe,arabicMatch as X}from"../../utils/arabic.js";import{daysUntil as be}from"../../utils/date.js";import{getCurrentUser as q}from"../../core/auth.js";import{openManzalaVoiceAssistantModal as ye}from"../../services/voice.service.js";import{executeFastSearch as _e,warmupSearchEngine as z}from"../../services/search-engine.service.js";import{getCategorySvg as ee}from"../../utils/professions-data.js";import{getCategoryVisualMeta as we,renderCategoryCardIcon as xe}from"../../utils/category-visual.js";import{resolveDeliveryVehicle as Ce}from"../../utils/delivery-vehicle.js";const Je={pharmacy:{emoji:"\u{1F48A}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},supermarket:{emoji:"\u{1F6D2}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},paint:{emoji:"\u{1F3A8}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},herbs:{emoji:"\u{1F33F}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},doctor:{emoji:"\u{1F468}\u200D\u2695\uFE0F",color:"rgba(41,128,185,0.1)",border:"#2980B9"},plumbing:{emoji:"\u{1F527}",color:"rgba(52,73,94,0.1)",border:"#52596E"},plumber:{emoji:"\u{1FAA0}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpenter:{emoji:"\u{1FA9A}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},tiler:{emoji:"\u{1F9F1}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},painter:{emoji:"\u{1F58C}\uFE0F",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},electrician:{emoji:"\u26A1",color:"rgba(243,156,18,0.1)",border:"#F39C12"},"ac-technician":{emoji:"\u2744\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},blacksmith:{emoji:"\u{1F6E0}\uFE0F",color:"rgba(52,73,94,0.1)",border:"#52596E"},alumital:{emoji:"\u{1FA9F}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},mechanic:{emoji:"\u{1F529}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},upholsterer:{emoji:"\u{1F6CB}\uFE0F",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},feed:{emoji:"\u{1F33E}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},poultry:{emoji:"\u{1F357}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},bakery:{emoji:"\u{1F35E}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},vegetables:{emoji:"\u{1F96C}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},antiques:{emoji:"\u{1F3FA}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},electronics:{emoji:"\u{1F4FA}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpet:{emoji:"\u{1F9F6}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},mattress:{emoji:"\u{1F6CF}\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},china:{emoji:"\u{1F37D}\uFE0F",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},electrical:{emoji:"\u{1F4A1}",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},roastery:{emoji:"\u{1F95C}",color:"rgba(101,67,33,0.1)",border:"#654321"},phones:{emoji:"\u{1F4F1}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},grocery:{emoji:"\u{1F3EA}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},hypermarket:{emoji:"\u{1F3EC}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"},delivery:{emoji:"\u{1F680}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"}},Ke={emoji:"\u{1F3EA}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"};export async function renderHomePage(n,{user:i}={}){const t=document.getElementById("hero-section-static");if(!!!t)n.innerHTML=ne(),oe();else{t.removeAttribute("id");const e=[];let c=t.nextSibling;for(;c;)e.push(c),c=c.nextSibling;e.forEach(u=>u.parentNode&&u.parentNode.removeChild(u));const d=document.createElement("div");d.innerHTML=ne();const v=d.querySelector(".hero");if(v){if(!t.querySelector(".hero-heritage-decor")){const u=v.querySelector(".hero-heritage-decor");u&&t.insertAdjacentElement("afterbegin",u)}v.remove()}for(;d.firstChild;)n.appendChild(d.firstChild);oe()}T();try{const e=w("categories_all"),c=w("published_100_");if(Array.isArray(e)&&e.length>0&&(ae(e),se(e)),Array.isArray(c)&&c.length>0){const d=q()||i;T(c);const v=U(c,d?.uid);F(v.slice(0,8)),re(c.length,e?.length||31),z(c,e||[])}}catch{}let s=[],r=[],l=[],g=[];try{const[e,c,d,v]=await Promise.allSettled([de(),Q({limit:100}),J(8),pe("homepage")]);s=e.status==="fulfilled"&&Array.isArray(e.value)&&e.value.length?e.value:w("categories_all")||R||[],r=c.status==="fulfilled"&&Array.isArray(c.value)&&c.value.length?c.value:w("published_100_")||[],l=d.status==="fulfilled"&&Array.isArray(d.value)?d.value:w("offers_active_8")||[],g=v.status==="fulfilled"&&Array.isArray(v.value)?v.value:w("ads_homepage")||[]}catch(e){console.warn("[Home] Data load non-fatal warning:",e),s=w("categories_all")||R||[],r=w("published_100_")||[],l=w("offers_active_8")||[],g=w("ads_homepage")||[]}const p=q()||i;if(typeof window<"u"&&Array.isArray(r)&&r.length>0){window._placesRegistry=window._placesRegistry||new Map;for(const e of r){if(!e)continue;const c=String(e.slug||e.id||e._key||"").toLowerCase().trim();c&&(window._placesRegistry.set(c,e),e.id&&window._placesRegistry.set(String(e.id).toLowerCase().trim(),e),e.slug&&window._placesRegistry.set(String(e.slug).toLowerCase().trim(),e))}}try{s&&s.length&&ae(s)}catch(e){console.warn("[Home] renderCategories err:",e)}try{T(r)}catch(e){console.warn("[Home] initHomeVerifiedShowcase err:",e)}try{const e=U(r,p?.uid);F(e.slice(0,8))}catch(e){console.warn("[Home] renderLatestPlaces err:",e)}try{l&&l.length&&ie(l)}catch(e){console.warn("[Home] renderOffers err:",e)}try{const e=(r||[]).filter(c=>{if(!c)return!1;if(c.deliveryType||c.categoryId?.includes("delivery"))return!0;const d=String(c.name||"").toLowerCase();return/توكتوك|تاكسي|شانجي|اتوبيس|توصيل|دليفري|وصلي/i.test(d)?!/صيدلية|مطعم|كشري|حلواني|سوبر\s*ماركت|هايبر/i.test(d):!1});e.length&&Le(e)}catch(e){console.warn("[Home] renderDeliveryServices err:",e)}try{g&&g.length&&Se(g)}catch(e){console.warn("[Home] renderAds err:",e)}try{re(r.length||0,s?.length||31)}catch{}try{z(r,s||[])}catch{}try{se(s||[])}catch{}try{Me()}catch{}Promise.resolve().then(()=>{try{import("../components/WhoIsAvailableNow.js").then(({renderWhoIsAvailableNow:e})=>{const c=document.getElementById("home-oncall-craftsmen-container");c&&e(c)}).catch(e=>console.warn("[Home] WhoIsAvailableNow load err:",e))}catch{}try{import("../components/ServiceRequestsSection.js").then(({renderServiceRequestsSection:e})=>{const c=document.getElementById("home-service-requests-container");c&&e(c,{limit:4,showHero:!1,isCompact:!0})}).catch(e=>console.warn("[Home] ServiceRequestsSection load err:",e))}catch{}try{import("../components/AroundMeRadar.js").then(({mountAroundMeRadar:e})=>{e("home-around-me-container")}).catch(e=>console.warn("[Home] AroundMeRadar load err:",e))}catch{}try{ve("home-sponsored-container",r,{title:"\u0623\u0645\u0627\u0643\u0646 \u0648\u0625\u0639\u0644\u0627\u0646\u0627\u062A \u0645\u0645\u064A\u0632\u0629 \u0641\u064A \u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",subtitle:"\u0623\u0646\u0634\u0637\u0629 \u062A\u062C\u0627\u0631\u064A\u0629 \u0648\u062E\u062F\u0645\u0627\u062A \u0645\u0648\u0635\u0649 \u0628\u0647\u0627 \u0648\u0645\u0639\u062A\u0645\u062F\u0629 \u0641\u064A \u0627\u0644\u0645\u062F\u064A\u0646\u0629",maxVisible:4})}catch(e){console.warn("[Home] SponsoredShowcase mount err:",e)}try{import("../components/WideAdsBanner.js").then(({mountWideAdsBanner:e})=>e("wide-ads-banner")).catch(e=>console.warn("[Home] WideAdsBanner load err:",e))}catch{}try{Be()}catch{}}),typeof window<"u"&&!window._homeRealtimeSyncAttached&&(window._homeRealtimeSyncAttached=!0,window.addEventListener("manzala:realtime_sync",async e=>{try{const[c,d]=await Promise.all([Q({limit:100,forceFresh:!0}).catch(()=>[]),J(8).catch(()=>[])]);if(c&&c.length){const v=q()||userArg,u=U(c,v?.uid);F(u.slice(0,8)),T(c),z(c,w("categories_all")||[])}d&&d.length&&ie(d)}catch(c){console.warn("[Home Realtime Sync Error]:",c)}}))}function U(n,i=null,t=!1){const a=new Set,s=[],r=[];return[...n].sort((p,e)=>{const c=Number(p.createdAt)||Number(p.updatedAt)||0;return(Number(e.createdAt)||Number(e.updatedAt)||0)-c}).forEach(p=>{const e=p._key||p.id;a.has(e)||(a.add(e),ue(p)?s.push(p):r.push(p))}),[...t?ke(s):s,...r]}function ae(n){const i=document.getElementById("categories-grid");if(!i)return;const t=Array.isArray(n)&&n.length>0?n:w("categories_all")||R||[];if(!t||!t.length)return i.querySelector(".category-card"),void 0;i.innerHTML=t.map(a=>{const s=a.slug||a._key||a.id||"",r=we(a),l=xe(a,{size:40});return`
    ...
    `}).join("")}const l=C.length,g=[];for(let p=0;p<Math.min(4,l);p++)g.push(C[(P+p)%l]);if(r(g),I&&(clearInterval(I),I=null),l>=2){let p=1;I=setInterval(()=>{i.querySelectorAll(".fair-place-card").forEach(c=>c.classList.add("anim-swap")),setTimeout(()=>{P=(P+1)%C.length,p++;const c=[],d=C.length;for(let v=0;v<Math.min(4,d);v++)c.push(C[(P+v)%d]);r(c),t&&(t.innerHTML=`\u{1F7E2} <b>\u062A\u0645 \u062A\u062F\u0648\u064A\u0631 \u0627\u0644\u0635\u062F\u0627\u0631\u0629 \u062A\u0644\u0642\u0627\u0626\u064A\u0627\u064B (${p}):</b> \u062A\u062A\u063A\u064A\u0631 \u0627\u0644\u0645\u0631\u0627\u0643\u0632 \u062F\u0648\u0631\u064A\u0627\u064B \u0644\u0636\u0645\u0627\u0646 \u062A\u0643\u0627\u0641\u0624 \u0646\u0633\u0628 \u0627\u0644\u0645\u0634\u0627\u0647\u062F\u0629 \u0644\u0643\u0627\u0641\u0629 \u0627\u0644\u0623\u0645\u0627\u0643\u0646 \u0627\u0644\u0645\u0648\u062B\u0642\u0629!`)},300)},4500)}}function ke(n){const i=[...n];for(let t=i.length-1;t>0;t--){const a=Math.floor(Math.random()*(t+1));[i[t],i[a]]=[i[a],i[t]]}return i}function F(n){const i=document.getElementById("latest-places-grid");if(i){if(!n||!n.length){if(i.querySelector(".fair-place-card")||i.querySelector(".place-card"))return;i.innerHTML=`
  ...
  `}).join("")}}function Se(n){const i=document.getElementById("ads-container");if(!i||!n||!n.length)return;const t=n.filter(a=>a&&(a.imageUrl||a.image_url));if(!t.length){i.innerHTML="";return}i.innerHTML=t.map(a=>{let s=(a.link||"#").trim();s.startsWith("place.html")&&(s="/"+s);const r=a.id||a._id||"",l=a.title||"\u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632",g=a.imageUrl||a.image_url||"";return`
  ...
  `,Ee(t)}function Ee(n){let i=!1,t=!1;function a(){if(t)return;t=!0;const s=n.querySelectorAll(".stats-bar__value[data-target]");if(!s.length){t=!1;return}const r=1800,l=performance.now();s.forEach(e=>{e.classList.remove("stats-done"),e.classList.add("stats-counting")});let g=-1;function p(e){const c=e-l,d=Math.min(1,c/r),v=1-Math.pow(1-d,3);s.forEach(u=>{const _=parseInt(u.getAttribute("data-target"),10)||0,m=u.getAttribute("data-prefix")||"",f=u.getAttribute("data-suffix")||"",$=Math.floor(v*_);if(u.textContent=`${m}${$.toLocaleString("en-US")}${f}`,$!==g){g=$;const x=420+$/Math.max(1,_)*430;A.playTick(x)}}),d<1?requestAnimationFrame(p):(s.forEach(u=>{const _=parseInt(u.getAttribute("data-target"),10)||0,m=u.getAttribute("data-prefix")||"",f=u.getAttribute("data-suffix")||"";u.textContent=`${m}${_.toLocaleString("en-US")}${f}`,u.classList.remove("stats-counting"),u.classList.add("stats-done")}),A.playDoneChime(),t=!1)}requestAnimationFrame(p)}if("IntersectionObserver"in window){const s=new IntersectionObserver(r=>{r.forEach(l=>{l.isIntersecting&&!i&&(i=!0,a(),s.disconnect())})},{threshold:.15});s.observe(n)}else a();n.querySelectorAll(".stats-interactive-item").forEach(s=>{s.addEventListener("click",()=>{A.init(),A.ctx&&A.ctx.state==="suspended"&&A.ctx.resume().catch(()=>{}),a()})})}function se(n){const i=document.getElementById("hero-search-glow-wrap"),t=document.getElementById("hero-search-input"),a=document.getElementById("hero-search-btn"),s=document.getElementById("hero-search-clear"),r=document.getElementById("hero-live-dropdown");if(!t)return;r&&!document.getElementById("hero-live-list")&&(r.innerHTML=`
    ...
    `);const l=document.getElementById("hero-live-list"),g=document.getElementById("hero-live-count"),p=document.getElementById("hero-live-all-btn"),e=document.getElementById("hero-quick-cats");e&&Array.isArray(n)&&n.length>0&&(e.innerHTML=n.slice(0,10).map(m=>{const f=m.slug||m._key||m.id||"",$=ee(f||m.name,18);return`
        <a href="category.html?slug=${encodeURIComponent(f)}" class="hero__quick-cat">
          ${$||m.icon||"\u{1F3EA}"} ${y(m.name)}
        </a>
      `}).join(""));function c(){const m=t.value.trim();m&&(window.location.href=`search.html?q=${encodeURIComponent(m)}`)}a?.addEventListener("click",m=>{m.stopPropagation(),c()}),t.addEventListener("keydown",m=>{m.key==="Enter"?c():m.key==="Escape"&&r?.classList.remove("visible")}),s?.addEventListener("click",m=>{m.stopPropagation(),t.value="",s.classList.remove("visible"),r?.classList.remove("visible"),l&&(l.innerHTML=""),t.focus()});function d(){!r||!l||(g&&(g.textContent="\u0645\u0642\u062A\u0631\u062D\u0627\u062A"),l.innerHTML=`
    ...
    `,l.querySelectorAll(".hero-live-suggestion-chip").forEach(m=>{m.addEventListener("click",f=>{f.preventDefault(),f.stopPropagation();const $=m.getAttribute("data-q")||"";t.value=$,t.dispatchEvent(new Event("input",{bubbles:!0})),t.focus()})}),r.classList.add("visible"))}t.addEventListener("focus",()=>{t.value.trim().length>=1&&l?.children.length>0?r?.classList.add("visible"):t.value.trim()||d()}),t.addEventListener("click",()=>{t.value.trim()||d()});let v=null,u=0;t.addEventListener("input",()=>{const m=t.value.trim();if(s?.classList.toggle("visible",m.length>0),p&&(p.href=`search.html?q=${encodeURIComponent(m)}`),!m){r?.classList.remove("visible"),l&&(l.innerHTML="");return}const f=m.length<=2?0:35;clearTimeout(v),v=setTimeout(async()=>{const $=++u;try{let V=function(b,h){if(!b)return"";if(!h)return y(b);const L=h.trim().split(/\s+/).filter(Boolean);if(!L.length)return y(b);const E=L.map(k=>k.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")),M=new RegExp(`(${E.join("|")})`,"gi");return y(b).replace(M,'<span class="search-highlight">$1</span>')};const x=await _e(m,{limit:6});if($!==u||!r||!l)return;if(!x||x.length===0){g&&(g.textContent="0"),l.innerHTML=`
          ...
          `,r.classList.add("visible");return}g&&(g.textContent=String(x.length));let O="";x.matchingCategories&&x.matchingCategories.length>0&&(O=`
            <div class="hero-live-matched-cats">
              ...
              <div class="hero-live-matched-cats__chips">
                ${x.matchingCategories.map(b=>{const h=b.slug||b.id||"",L=ee(h||b.name,16);return`
          ...
          `),l.innerHTML=O+x.map(b=>{const h=b.raw||b,L=h.name||"\u0645\u0643\u0627\u0646 \u0628\u0627\u0644\u062F\u0644\u064A\u0644",E=h.categoryName||b.category||"",M=h.area||h.address||"\u0645\u062F\u064A\u0646\u0629 \u0627\u0644\u0645\u0646\u0632\u0644\u0629",k=h.slug||h.id||"",B=h.photoURL||h.logo||h.coverURL||h.coverImageUrl||h.logoUrl||"",ce=h.isVerified||!1,Z=h.isOpen!==void 0?h.isOpen:!0,N=(L.trim()[0]||"\u0645").toUpperCase(),j=(h.phone||"").trim(),W=(h.whatsapp||h.phone||"").trim(),S=W?W.replace(/[^0-9]/g,""):"",H=S?S.startsWith("2")?S:S.startsWith("0")?"2"+S:"20"+S:"";let G="";if((j||H)&&(G=`
            ...
            `),typeof window<"u"&&window._placesRegistry&&k){const le=String(k).toLowerCase().trim();window._placesRegistry.set(le,h),h.slug&&window._placesRegistry.set(String(h.slug).toLowerCase().trim(),h),h.id&&window._placesRegistry.set(String(h.id).toLowerCase().trim(),h)}return`
            <a href="/place.html?slug=${encodeURIComponent(k)}" class="hero-live-dropdown__item" role="option"
              ...
              <div class="hero-live-content">
                <div class="hero-live-title-row">
                  <span class="hero-live-name">${V(L,m)}</span>
                  ${ce?'<span class="hero-live-verified" title="\u0645\u0643\u0627\u0646 \u0645\u0648\u062B\u0642">\u2713</span>':""}
                </div>
                <div class="hero-live-meta-row">
                  ${E?`<span class="hero-live-cat">${V(E,m)}</span>`:""}
                  <span class="hero-live-area">${y(M)}</span>
