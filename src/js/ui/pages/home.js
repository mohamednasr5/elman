import{getCategories as U,getPublishedPlaces as V,getActiveOffers as D,getAds as Z}from"../../core/db.js";import{WORKER_URL as A}from"../../core/firebase.js";import"../../core/state.js";import{renderPlaceCard as N,renderPlaceCardSkeleton as O}from"../components/PlaceCard.js";import{isAtmPlace as W}from"../../utils/atm.js";import{mountSponsoredShowcase as Q,isPlaceSponsored as G}from"../components/SponsoredShowcase.js";import{formatPrice as T,calcDiscount as J,arabicMatch as I}from"../../utils/arabic.js";import{daysUntil as K}from"../../utils/date.js";import{getCurrentUser as Y}from"../../core/auth.js";import{openManzalaVoiceAssistantModal as X}from"../../services/voice.service.js";import{executeFastSearch as ee}from"../../services/search-engine.service.js";import{getCategorySvg as ae}from"../../utils/professions-data.js";import{getCategoryVisualMeta as te,renderCategoryCardIcon as ie}from"../../utils/category-visual.js";const Pe={pharmacy:{emoji:"\u{1F48A}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},supermarket:{emoji:"\u{1F6D2}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},paint:{emoji:"\u{1F3A8}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},herbs:{emoji:"\u{1F33F}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},doctor:{emoji:"\u{1F468}\u200D\u2695\uFE0F",color:"rgba(41,128,185,0.1)",border:"#2980B9"},plumbing:{emoji:"\u{1F527}",color:"rgba(52,73,94,0.1)",border:"#52596E"},plumber:{emoji:"\u{1FAA0}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpenter:{emoji:"\u{1FA9A}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},tiler:{emoji:"\u{1F9F1}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},painter:{emoji:"\u{1F58C}\uFE0F",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},electrician:{emoji:"\u26A1",color:"rgba(243,156,18,0.1)",border:"#F39C12"},"ac-technician":{emoji:"\u2744\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},blacksmith:{emoji:"\u{1F6E0}\uFE0F",color:"rgba(52,73,94,0.1)",border:"#52596E"},alumital:{emoji:"\u{1FA9F}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},mechanic:{emoji:"\u{1F529}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},upholsterer:{emoji:"\u{1F6CB}\uFE0F",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},feed:{emoji:"\u{1F33E}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},poultry:{emoji:"\u{1F357}",color:"rgba(243,156,18,0.1)",border:"#F39C12"},bakery:{emoji:"\u{1F35E}",color:"rgba(230,126,34,0.1)",border:"#E67E22"},vegetables:{emoji:"\u{1F96C}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},antiques:{emoji:"\u{1F3FA}",color:"rgba(149,165,166,0.1)",border:"#95A5A6"},electronics:{emoji:"\u{1F4FA}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},carpet:{emoji:"\u{1F9F6}",color:"rgba(155,89,182,0.1)",border:"#9B59B6"},mattress:{emoji:"\u{1F6CF}\uFE0F",color:"rgba(52,152,219,0.1)",border:"#3498DB"},china:{emoji:"\u{1F37D}\uFE0F",color:"rgba(231,76,60,0.1)",border:"#E74C3C"},electrical:{emoji:"\u{1F4A1}",color:"rgba(241,196,15,0.1)",border:"#F1C40F"},roastery:{emoji:"\u{1F95C}",color:"rgba(101,67,33,0.1)",border:"#654321"},phones:{emoji:"\u{1F4F1}",color:"rgba(41,128,185,0.1)",border:"#2980B9"},grocery:{emoji:"\u{1F3EA}",color:"rgba(39,174,96,0.1)",border:"#27AE60"},hypermarket:{emoji:"\u{1F3EC}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"},delivery:{emoji:"\u{1F680}",color:"rgba(231,76,60,0.1)",border:"#E74C3C"}},ze={emoji:"\u{1F3EA}",color:"rgba(27,79,114,0.1)",border:"#1B4F72"};export async function renderHomePage(n,{user:t}={}){const e=document.getElementById("hero-section-static");if(!!!e)n.innerHTML=H(),z();else{e.removeAttribute("id");const i=[];let r=e.nextSibling;for(;r;)i.push(r),r=r.nextSibling;i.forEach(l=>l.parentNode&&l.parentNode.removeChild(l));const c=document.createElement("div");c.innerHTML=H();const m=c.querySelector(".hero");for(m&&m.remove();c.firstChild;)n.appendChild(c.firstChild);z()}P();try{const[i,r,c,m]=await Promise.all([U(),V({limit:100}),D(8),Z("homepage")]),l=Y()||t,o=r||[];if(typeof window<"u"&&Array.isArray(o)){window._placesRegistry=window._placesRegistry||new Map;for(const s of o){if(!s)continue;const p=String(s.slug||s.id||s._key||"").toLowerCase().trim();p&&(window._placesRegistry.set(p,s),s.id&&window._placesRegistry.set(String(s.id).toLowerCase().trim(),s),s.slug&&window._placesRegistry.set(String(s.slug).toLowerCase().trim(),s))}}oe(i||[]),P(o);const h=re(o,l?.uid);ce(h.slice(0,8)),le(c||[]),de(o.filter(s=>s.categoryId?.includes("delivery")||s.deliveryType)),pe(m||[]),ge(o.length||0,i?.length||31),ve(i||[]),fe(),Promise.resolve().then(()=>{import("../components/WhoIsAvailableNow.js").then(({renderWhoIsAvailableNow:s})=>{const p=document.getElementById("home-oncall-craftsmen-container");p&&s(p)}).catch(()=>{}),import("../components/ServiceRequestsSection.js").then(({renderServiceRequestsSection:s})=>{const p=document.getElementById("home-service-requests-container");p&&s(p,{limit:4,showHero:!0,isCompact:!0})}).catch(()=>{}),import("../components/AroundMeRadar.js").then(({mountAroundMeRadar:s})=>{s("home-around-me-container")}).catch(()=>{}),Q("home-sponsored-container",o,{title:"\u0623\u0645\u0627\u0643\u0646 \u0648\u0625\u0639\u0644\u0627\u0646\u0627\u062A \u0645\u0645\u064A\u0632\u0629 \u0641\u064A \u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",subtitle:"\u0623\u0646\u0634\u0637\u0629 \u062A\u062C\u0627\u0631\u064A\u0629 \u0648\u062E\u062F\u0645\u0627\u062A \u0645\u0648\u0635\u0649 \u0628\u0647\u0627 \u0648\u0645\u0639\u062A\u0645\u062F\u0629 \u0641\u064A \u0627\u0644\u0645\u062F\u064A\u0646\u0629",maxVisible:4});try{import("../components/WideAdsBanner.js").then(({mountWideAdsBanner:s})=>s("wide-ads-banner")).catch(()=>{})}catch{}ue()})}catch{}}function re(n,t=null,e=!1){const a=new Set,i=[],r=[];return[...n].sort((l,o)=>{const h=Number(l.createdAt)||Number(l.updatedAt)||0;return(Number(o.createdAt)||Number(o.updatedAt)||0)-h}).forEach(l=>{const o=l._key||l.id;a.has(o)||(a.add(o),G(l)?i.push(l):r.push(l))}),[...e?se(i):i,...r]}function oe(n){const t=document.getElementById("categories-grid");!t||!n||(t.innerHTML=n.map(e=>{const a=e.slug||e._key||e.id||"",i=te(e),r=ie(e,{size:40});return`
    ...
    `}).join(""))}const j="manzala_verified_showcase_v1";let C=null,_=[],k=0;const ne=[{id:"p_1788703900620_oae8ka",slug:"mtam-basl-wbaha-llmakwlat-albhrya",name:"\u0645\u0637\u0639\u0645 \u0628\u0627\u0633\u0644 \u0648\u0628\u0627\u0647\u0649 \u0644\u0644\u0645\u0623\u0643\u0648\u0644\u0627\u062A \u0627\u0644\u0628\u062D\u0631\u064A\u0629",area:"\u0627\u0644\u0645\u0637\u0631\u064A\u0629 \u062F\u0642\u0647\u0644\u064A\u0629",category:"\u0645\u0637\u0627\u0639\u0645 \u0648\u0623\u0633\u0645\u0627\u0643",cover:"https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"-P03LX9MledW_z7QfyHO",slug:"-P03LX9MledW_z7QfyHO",name:"\u0627\u0644\u062D\u0633\u0646 \u0644\u0635\u064A\u0627\u0646\u0629 \u0627\u0644\u0647\u0648\u0627\u062A\u0641 \u0627\u0644\u0645\u062D\u0645\u0648\u0644\u0629",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0634\u0627\u0631\u0639 \u0627\u0644\u0628\u062D\u0631",category:"\u0635\u064A\u0627\u0646\u0629 \u0648\u0645\u0648\u0628\u0627\u064A\u0644",cover:"https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!0},{id:"p_1788801925745_vuxmjs",slug:"mtbkh-eyma-llaakl-albyty",name:"\u0645\u0637\u0628\u062E \u0625\u064A\u0645\u0649 \u0644\u0644\u0623\u0643\u0644 \u0627\u0644\u0628\u064A\u062A\u064A",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u0637\u0631\u064A\u0642 \u0627\u0644\u0645\u0646\u0635\u0648\u0631\u0629",category:"\u0623\u0643\u0644 \u0628\u064A\u062A\u064A \u0648\u062D\u0644\u0648\u064A\u0627\u062A",cover:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1},{id:"-P0hEa0K6ZfAM65O27G9",slug:"kwafyr-mnh-asad",name:"\u0643\u0648\u0627\u0641\u064A\u0631 \u0645\u0646\u0647 \u0623\u0633\u0639\u062F",area:"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 - \u062D\u064A \u0627\u0644\u0633\u0644\u0627\u0645",category:"\u0628\u064A\u0648\u062A\u064A \u0648\u0643\u0648\u0627\u0641\u064A\u0631",cover:"https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=300&h=180&q=75",isSponsored:!1}];function P(n=null){const t=document.getElementById("home-verified-cards-grid"),e=document.getElementById("home-verified-status-text");if(!t)return;let a=null;try{const l=localStorage.getItem(j);if(l){const o=JSON.parse(l);Array.isArray(o)&&o.length>0&&(a=o)}}catch{}if(Array.isArray(n)&&n.length>0){const l=n.filter(o=>o&&o.isVerified&&!W(o)).map(o=>({id:o.id||o._key,slug:o.slug||o.id,name:o.name,area:o.area||"\u0627\u0644\u0645\u0646\u0632\u0644\u0629 \u0648\u0627\u0644\u0645\u0637\u0631\u064A\u0629",category:o.categoryName||o.customCategory||o.categoryId||"\u0646\u0634\u0627\u0637 \u062A\u062C\u0627\u0631\u064A",cover:o.coverImageUrl||o.logoUrl||o.gallery&&o.gallery[0]||"/assets/images/og-whatsapp.jpg",isSponsored:!!(o.isSponsored&&(!o.sponsoredUntil||o.sponsoredUntil>Date.now()))}));if(l.length>0){_=l;try{localStorage.setItem(j,JSON.stringify(l))}catch{}}}else _.length||(a&&a.length>0?_=a:_=[...ne]);const i=["\u{1F947} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #1","\u{1F948} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #2","\u{1F949} \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #3","\u{1F396}\uFE0F \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #4"];function r(l){t.innerHTML=l.map((o,h)=>{const s=o.slug||o.id||"";return`
      <article class="fair-place-card" data-card-index="${h}"
               onclick="window.__openPlaceCard ? window.__openPlaceCard(this, '${g(s)}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(s)}')"
               onpointerdown="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(s)}')"
               onmouseenter="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(s)}')"
               style="cursor:pointer">
        <span class="fair-place-card__rank">${i[h]||`\u{1F396}\uFE0F \u0627\u0644\u0635\u062F\u0627\u0631\u0629 #${h+1}`}</span>
        <div class="fair-place-card__cover">
          <img src="${g(o.cover)}" alt="${g(o.name)}" loading="lazy" onerror="this.src='/assets/images/og-whatsapp.jpg'">
          <div class="fair-place-card__badges">
            ${o.isSponsored?'<span class="fair-badge-sponsored">\u2B50 \u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632</span>':""}
            <span class="fair-badge-verified">\u2713 \u0645\u0648\u062B\u0642 \u0631\u0633\u0645\u064A\u0627\u064B</span>
          </div>
        </div>
        <div class="fair-place-card__body">
          <h3 class="fair-place-card__title" title="${g(o.name)}">${y(o.name)}</h3>
          <div class="fair-place-card__meta">
            <span>\u{1F4CD} ${y(o.area)}</span>
            <span>\u{1F3F7}\uFE0F ${y(o.category)}</span>
          </div>
          <a href="/place.html?slug=${encodeURIComponent(s)}" class="fair-place-card__link" onclick="event.preventDefault(); window.__openPlaceCard ? window.__openPlaceCard(this, '${g(s)}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(s)}')">\u0639\u0631\u0636 \u0628\u0637\u0627\u0642\u0629 \u0627\u0644\u0645\u0643\u0627\u0646 \u2197</a>
        </div>
      </article>
    `}).join("")}const c=_.length,m=[];for(let l=0;l<Math.min(4,c);l++)m.push(_[(k+l)%c]);if(r(m),C&&(clearInterval(C),C=null),c>=2){let l=1;C=setInterval(()=>{t.querySelectorAll(".fair-place-card").forEach(h=>h.classList.add("anim-swap")),setTimeout(()=>{k=(k+1)%_.length,l++;const h=[],s=_.length;for(let p=0;p<Math.min(4,s);p++)h.push(_[(k+p)%s]);r(h),e&&(e.innerHTML=`\u{1F7E2} <b>\u062A\u0645 \u062A\u062F\u0648\u064A\u0631 \u0627\u0644\u0635\u062F\u0627\u0631\u0629 \u062A\u0644\u0642\u0627\u0626\u064A\u0627\u064B (${l}):</b> \u062A\u062A\u063A\u064A\u0631 \u0627\u0644\u0645\u0631\u0627\u0643\u0632 \u062F\u0648\u0631\u064A\u0627\u064B \u0644\u0636\u0645\u0627\u0646 \u062A\u0643\u0627\u0641\u0624 \u0646\u0633\u0628 \u0627\u0644\u0645\u0634\u0627\u0647\u062F\u0629 \u0644\u0643\u0627\u0641\u0629 \u0627\u0644\u0623\u0645\u0627\u0643\u0646 \u0627\u0644\u0645\u0648\u062B\u0642\u0629!`)},300)},4500)}}function se(n){const t=[...n];for(let e=t.length-1;e>0;e--){const a=Math.floor(Math.random()*(e+1));[t[e],t[a]]=[t[a],t[e]]}return t}function ce(n){const t=document.getElementById("latest-places-grid");if(t){if(!n||!n.length){t.innerHTML=`
    ...
    `;return}t.innerHTML=n.map(e=>N(e)).join("")}}function le(n){const t=document.getElementById("offers-scroll"),e=document.getElementById("offers-section");if(t){if(!n||!n.length){e?.remove();return}t.innerHTML=n.map(a=>{const i=a.discountPercent||J(a.oldPrice,a.newPrice),r=K(a.endDate);return`
      <article class="offer-card"
               onclick="window.__openPlaceCard ? window.__openPlaceCard(this, '${g(a.placeSlug||"")}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(a.placeSlug||"")}')"
               onpointerdown="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(a.placeSlug||"")}')"
               onmouseenter="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(a.placeSlug||"")}')"
               style="cursor:pointer">
        <div class="offer-card__image">
          ${a.imageUrl?`<img src="${g(a.imageUrl)}" alt="${g(a.title)}" loading="lazy" />`:'<div style="width:100%;height:100%;background:var(--primary-alpha);display:flex;align-items:center;justify-content:center;font-size:2rem">\u{1F3F7}\uFE0F</div>'}
        ...
        <div class="offer-card__body">
          ...
          <div class="offer-card__price">
            <span class="offer-card__price-new">${T(a.newPrice)}</span>
            ${a.oldPrice?`<span class="offer-card__price-old">${T(a.oldPrice)}</span>`:""}
          </div>
          `:""}
    ...
    `}).join("")}}function de(n){const t=document.getElementById("delivery-grid"),e=document.getElementById("delivery-section");if(!t)return;if(!n||!n.length){e?.remove();return}const a={motorcycle:"\u{1F3CD}\uFE0F",tuktuk:"\u{1F6FA}",car:"\u{1F697}",taxi:"\u{1F695}",bus:"\u{1F68C}",pickup_quarter:"\u{1F6FB}",pickup_half:"\u{1F69A}",truck_heavy:"\u{1F69B}",cart_donkey:"\u{1FACF}",cart_horse:"\u{1F40E}",loader:"\u{1F69C}",tractor_trailer:"\u{1F69C}"};t.innerHTML=n.slice(0,6).map(i=>{const r=i.slug||i._key||i.id||"";return`
    <a href="/place.html?slug=${encodeURIComponent(r)}" class="delivery-card"
       onclick="event.preventDefault(); window.__openPlaceCard ? window.__openPlaceCard(this, '${g(r)}', event) : (window.location.href='/place.html?slug=${encodeURIComponent(r)}')"
       onpointerdown="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(r)}')"
       onmouseenter="window.__prefetchPlaceCard && window.__prefetchPlaceCard('${g(r)}')">
      <div class="delivery-card__icon">${a[i.deliveryType]||"\u{1F680}"}</div>
      <div class="delivery-card__info">
        <div class="delivery-card__name">${y(i.name)}</div>
        <div class="delivery-card__type">\u062E\u062F\u0645\u0629 \u062A\u0648\u0635\u064A\u0644 \u0628\u0627\u0644\u0645\u0646\u0632\u0644\u0629</div>
      </div>
    </a>
  `}).join("")}function pe(n){const t=document.getElementById("ads-container");if(!t||!n||!n.length)return;const e=n.filter(a=>a&&(a.imageUrl||a.image_url));if(!e.length){t.innerHTML="";return}t.innerHTML=e.map(a=>{let i=(a.link||"#").trim();i.startsWith("place.html")&&(i="/"+i);const r=a.id||a._id||"",c=a.title||"\u0625\u0639\u0644\u0627\u0646 \u0645\u0645\u064A\u0632",m=a.imageUrl||a.image_url||"";return`
      <a href="${g(i)}" class="ad-banner" target="_blank" rel="noopener noreferrer sponsored" aria-label="${g(c)}" data-ad-id="${g(r)}">
        ...
        <img src="${g(m)}" alt="${g(c)}" loading="lazy" decoding="async"
             width="800" height="420" style="aspect-ratio:16/7;" />
      </a>
    `}).join(""),t.querySelectorAll(".ad-banner").forEach(a=>{a.addEventListener("click",()=>{const i=a.getAttribute("data-ad-id");if(i&&A)try{fetch(`${A}/api/ads/track-click`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:i}),keepalive:!0}).catch(()=>{})}catch{}})})}class me{constructor(){this.ctx=null,this.lastTickTime=0}init(){if(!this.ctx&&typeof window<"u"){const t=window.AudioContext||window.webkitAudioContext;t&&(this.ctx=new t)}}playTick(t=550){try{if(!this.ctx||this.ctx.state!=="running")return;const e=this.ctx.currentTime;if(e-this.lastTickTime<.038)return;this.lastTickTime=e;const a=this.ctx.createOscillator(),i=this.ctx.createGain();a.type="sine",a.frequency.setValueAtTime(t,e),a.frequency.exponentialRampToValueAtTime(t*1.35,e+.018),i.gain.setValueAtTime(.04,e),i.gain.exponentialRampToValueAtTime(1e-4,e+.022),a.connect(i),i.connect(this.ctx.destination),a.start(e),a.stop(e+.028)}catch{}}playDoneChime(){try{if(!this.ctx||this.ctx.state!=="running")return;const t=this.ctx.currentTime;[1046.5,1318.51,1567.98,2093].forEach((a,i)=>{const r=this.ctx.createOscillator(),c=this.ctx.createGain();r.type="triangle",r.frequency.setValueAtTime(a,t+i*.055),c.gain.setValueAtTime(.06,t+i*.055),c.gain.exponentialRampToValueAtTime(1e-4,t+i*.055+.32),r.connect(c),c.connect(this.ctx.destination),r.start(t+i*.055),r.stop(t+i*.055+.35)})}catch{}}}const w=new me;function ge(n,t){const e=document.getElementById("stats-bar");if(!e)return;const a=Math.max(191,Number(n)||0),i=Math.max(94,Number(t)||0),r=55;e.innerHTML=`
    <div class="stats-bar__inner container">
      ...
      <div class="stats-bar__item stats-interactive-item" title="\u0627\u0644\u0645\u062F\u0646 \u0648\u0627\u0644\u0642\u0631\u0649 \u0627\u0644\u0645\u0633\u062C\u0644\u0629 \u0628\u0627\u0644\u062F\u0644\u064A\u0644">
        <div class="stats-bar__value" data-target="${r}" data-prefix="+" data-suffix="">+${r}</div>
  ...
  `,he(e)}function he(n){let t=!1,e=!1;function a(){if(e)return;e=!0;const i=n.querySelectorAll(".stats-bar__value[data-target]");if(!i.length){e=!1;return}const r=1800,c=performance.now();i.forEach(o=>{o.classList.remove("stats-done"),o.classList.add("stats-counting")});let m=-1;function l(o){const h=o-c,s=Math.min(1,h/r),p=1-Math.pow(1-s,3);i.forEach(v=>{const d=parseInt(v.getAttribute("data-target"),10)||0,b=v.getAttribute("data-prefix")||"",f=v.getAttribute("data-suffix")||"",x=Math.floor(p*d);if(v.textContent=`${b}${x.toLocaleString("en-US")}${f}`,x!==m){m=x;const u=420+x/Math.max(1,d)*430;w.playTick(u)}}),s<1?requestAnimationFrame(l):(i.forEach(v=>{const d=parseInt(v.getAttribute("data-target"),10)||0,b=v.getAttribute("data-prefix")||"",f=v.getAttribute("data-suffix")||"";v.textContent=`${b}${d.toLocaleString("en-US")}${f}`,v.classList.remove("stats-counting"),v.classList.add("stats-done")}),w.playDoneChime(),e=!1)}requestAnimationFrame(l)}if("IntersectionObserver"in window){const i=new IntersectionObserver(r=>{r.forEach(c=>{c.isIntersecting&&!t&&(t=!0,a(),i.disconnect())})},{threshold:.15});i.observe(n)}else a();n.querySelectorAll(".stats-interactive-item").forEach(i=>{i.addEventListener("click",()=>{w.init(),w.ctx&&w.ctx.state==="suspended"&&w.ctx.resume().catch(()=>{}),a()})})}function ve(n){const t=document.getElementById("hero-search-glow-wrap"),e=document.getElementById("hero-search-input"),a=document.getElementById("hero-search-btn"),i=document.getElementById("hero-search-clear"),r=document.getElementById("hero-live-dropdown"),c=document.getElementById("hero-live-list"),m=document.getElementById("hero-live-count"),l=document.getElementById("hero-live-all-btn");if(!e)return;const o=document.getElementById("hero-quick-cats");o&&n&&(o.innerHTML=n.slice(0,10).map(d=>{const b=d.slug||d._key||d.id||"",f=ae(b||d.name,18);return`
        <a href="category.html?slug=${encodeURIComponent(b)}" class="hero__quick-cat">
          ${f||d.icon||"\u{1F3EA}"} ${y(d.name)}
        </a>
      `}).join(""));function h(){const d=e.value.trim();d&&(window.location.href=`search.html?q=${encodeURIComponent(d)}`)}a?.addEventListener("click",d=>{d.stopPropagation(),h()}),e.addEventListener("keydown",d=>{d.key==="Enter"?h():d.key==="Escape"&&r?.classList.remove("visible")}),i?.addEventListener("click",d=>{d.stopPropagation(),e.value="",i.classList.remove("visible"),r?.classList.remove("visible"),c&&(c.innerHTML=""),e.focus()}),e.addEventListener("focus",()=>{e.value.trim().length>=1&&c?.children.length>0&&r?.classList.add("visible")});let s=null,p=0;e.addEventListener("input",()=>{const d=e.value.trim();if(i?.classList.toggle("visible",d.length>0),l&&(l.href=`search.html?q=${encodeURIComponent(d)}`),!d){r?.classList.remove("visible"),c&&(c.innerHTML="");return}clearTimeout(s),s=setTimeout(async()=>{const b=++p;try{const f=await ee(d,{limit:6});if(b!==p||!r||!c)return;if(!f||f.length===0){m&&(m.textContent="0"),c.innerHTML=`
    ...
    <section class="section" style="background:var(--surface);padding-block:var(--space-8);border-bottom:1px solid var(--border)">
      <div class="container">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(130px, 1fr));gap:10px;margin-top:14px" id="villages-grid-container">
          ${n.map(t=>`
            <a href="${{\u0627\u0644\u0639\u0632\u064A\u0632\u0629:"#/village/al-aziza",\u0627\u0644\u0628\u0635\u0631\u0627\u0637:"#/village/al-basrat",\u0627\u0644\u0634\u0628\u0648\u0644:"#/village/al-shabboul",\u0627\u0644\u0639\u0635\u0627\u0641\u0631\u0629:"#/village/al-asafra",\u0627\u0644\u0646\u0633\u0627\u064A\u0645\u0629:"#/village/al-nasayma"}[t.name]||`places.html?area=${encodeURIComponent(t.name)}`}" class="category-card village-grid-item" data-name="${g(t.name)}" style="padding:12px 8px;text-align:center;text-decoration:none;border-radius:var(--radius-md);transition:all 0.2s ease;display:flex;flex-direction:column;align-items:center" title="\u062F\u0644\u064A\u0644 \u0623\u0645\u0627\u0643\u0646 \u0648\u062E\u062F\u0645\u0627\u062A \u0648\u0645\u0648\u0627\u0635\u0644\u0627\u062A ${t.name}">
  ...
  `}function y(n){return n?String(n).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}function g(n){return n?String(n).replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}function ue(){try{let i=function(){try{localStorage.setItem("__has_seen_welcome_video_v1","true")}catch{}e&&e.pause(),t.style.opacity="0";const r=t.querySelector(".first-visit-video-card");r&&(r.style.transform="scale(0.92)"),setTimeout(()=>{t&&t.parentNode&&t.parentNode.removeChild(t)},350)};return}catch(n){console.warn("[Welcome Video] error:",n)}}
