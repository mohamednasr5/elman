...
export async function renderDashboard($container, { user, section = 'overview', placeId = null }) {
    ...
    <div class="dashboard-layout">
      ...
      <aside class="dashboard-sidebar" role="navigation" aria-label="لوحة التحكم">
        ...
        <nav class="dashboard-sidebar__nav" id="dashboard-sidebar-nav">
          ...
          <a href="wallet.html" class="dashboard-nav-item dashboard-nav-item--wallet" style="background:linear-gradient(135deg,rgba(245,166,35,0.14) 0%,rgba(217,119,6,0.18) 100%);color:#B45309;font-weight:900;border:1.5px solid rgba(245,166,35,0.45);box-shadow:0 3px 10px rgba(245,166,35,0.14);border-radius:12px;margin:6px 0;display:flex;align-items:center;justify-content:space-between;padding:10px 14px">
            <div style="display:flex;align-items:center;gap:8px">
              <span class="dashboard-nav-item__icon" style="font-size:20px;line-height:1">🪙</span>
              <span style="font-size:13.5px;color:#92400E">محفظة الذهبيات</span>
            </div>
            <span class="badge" id="sidebar-coins-balance-badge" style="background:#F5A623;color:#0B1E30;font-size:12px;font-weight:900;padding:3px 9px;border-radius:9999px;box-shadow:0 2px 6px rgba(245,166,35,0.25)">
              ${getStoredCoinsBalance().toLocaleString('ar-EG')} ذهبية
            </span>
          </a>
  ...
  fetchLiveCoinsBalance().then(bal => {
...
async function renderOverviewSection($container, user) {
  ...
  bindPlaceItemActionListeners($container, user, places, () => renderOverviewSection($container, user));
}
...
async function renderPlacesSection($container, user) {
  ...
  bindPlaceItemActionListeners($container, user, places, () => renderPlacesSection($container, user));
}

function bindPlaceItemActionListeners($container, user, places, onRefresh) {
...
function bindPromoteConfirmAction(modal, placeId, placeName, onRefresh) {
  const confirmBtn = document.getElementById('btn-modal-confirm-promote');
  if (confirmBtn) {
    confirmBtn.addEventListener('click', async () => {
      confirmBtn.disabled = true;
      confirmBtn.innerHTML = '⏳ جاري التفعيل...';
      try {
        ...
        if (res.success) {
          ...
          confirmBtn.innerHTML = '🚀 تفعيل الإعلان المميز الآن (500 ذهبية)';
        }
      } catch (err) {
    ...
    <div style="padding:14px 10px;text-align:center">
      ...
      <div style="background:rgba(245,166,35,0.08);border:1.5px solid rgba(245,166,35,0.3);border-radius:12px;padding:10px 14px;margin-bottom:18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        ...
        <div style="text-align:left">
          ...
          <strong id="modal-promote-bal-display" style="font-size:15px;color:${hasEnough ? '#059669' : '#DC2626'}">
...
function openVerifyPlaceModal(placeId, placeName, user, onRefresh) {
  ...
  const contentHtml = `
    <div style="padding:14px 10px;text-align:center">
      ...
      <h3 style="font-size:17px;font-weight:900;color:var(--text-primary,#0F172A);margin:0 0 6px 0">
        توثيق نشاطك (${escHtml(placeName)}) بالعلامة الزرقاء
      </h3>
      <p style="font-size:12.5px;color:var(--text-muted,#64748B);line-height:1.6;margin:0 0 16px 0">
        احصل على شارة التوثيق الرسمية المعتمدة لزيادة ثقة العملاء والأولوية في الظهور مدى الحياة.
      </p>
      ...
      <div style="background:var(--surface-2,#F8FAFC);border:1px solid var(--border,#E2E8F0);border-radius:14px;padding:12px 14px;text-align:right;margin-bottom:16px;display:flex;flex-direction:column;gap:10px">
        ...
        <div style="display:flex;align-items:flex-start;gap:10px">
          <span style="font-size:18px">⭐</span>
          <div style="font-size:12.5px;color:var(--text-primary,#1E293B)">
            <strong>أسبقية في نتائج البحث:</strong> تفضيل الأماكن الموثقة في كافة التصنيفات والبحث الذكي.
          </div>
        </div>
        <div style="display:flex;align-items:flex-start;gap:10px">
          <span style="font-size:18px">🎁</span>
      ...
      <div style="background:rgba(37,99,235,0.06);border:1.5px solid rgba(37,99,235,0.25);border-radius:12px;padding:10px 14px;margin-bottom:18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:gap;gap:8px">
        <div style="text-align:right">
        ...
        <div style="text-align:left">
          <span style="font-size:11px;color:var(--text-muted);display:block">رصيدك الحالي</span>
          <strong id="modal-verify-bal-display" style="font-size:15px;color:${hasEnough ? '#059669' : '#DC2626'}">
            ${storedBal.toLocaleString('ar-EG')} ذهبية ${hasEnough ? '✓' : '⚠️'}
          </strong>
        </div>
      </div>
...
// Global delegated fallback safety net for place action buttons in dashboard
if (typeof document !== 'undefined' && !window.__dashboardPlaceActionsBound) {
  window.__dashboardPlaceActionsBound = true;
  document.addEventListener('click', (e) => {
    const promoteBtn = e.target.closest('.btn-action-promote-place');
    if (promoteBtn) {
      e.preventDefault();
      e.stopPropagation();
      const placeId = promoteBtn.dataset.placeId;
      const placeName = promoteBtn.dataset.placeName || 'المكان';
      openPromotePlaceModal(placeId, placeName, null, () => {
        window.location.reload();
      });
      return;
    }
    const verifyBtn = e.target.closest('.btn-action-verify-place');
    if (verifyBtn) {
      e.preventDefault();
      e.stopPropagation();
      const placeId = verifyBtn.dataset.placeId;
      const placeName = verifyBtn.dataset.placeName || 'المكان';
      openVerifyPlaceModal(placeId, placeName, null, () => {
        window.location.reload();
      });
      return;
    }
...
/**
 * Reusable Area Selector (55 villages + instant search + other pill + badge)
 * Used for Main Place, Branch Rows, and Quick Edit Branch Modals
 */
function renderAreaSelectForm({
  prefix = 'p',
  initialArea = 'المنزلة',
  label = 'المنطقة داخل المنزلة / المطرية',
  isRequired = true,
  placeholder = '🔍 ابحث بالاسم عن قريتك أو منطقتك (55 قرية ومدينة)...'
} = {}) {
  const currentArea = (initialArea || '').trim() || 'المنزلة';
  ...
  return `
    <div class="form-group area-picker-container" data-area-prefix="${escAttr(prefix)}" style="background:var(--surface-2);border:1.5px solid var(--primary);border-radius:var(--radius-lg);padding:var(--space-4);margin-bottom:var(--space-3)">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--space-2);flex-wrap:wrap;gap:6px">
        <label class="form-label" style="margin-bottom:0;font-weight:var(--font-weight-bold);color:var(--primary)">${escHtml(label)} ${isRequired ? '<span class="required">*</span>' : ''}</label>
        <div class="area-selected-badge" style="font-size:12px;color:var(--primary);display:flex;align-items:center;gap:6px">
          <span>المختار:</span>
          <span class="chip chip--primary area-selected-name" id="${prefix}-selected-area-name" style="font-weight:700">${escHtml(currentAreaName)}</span>
        </div>
      </div>
      ...
      <div style="position:relative;margin-bottom:10px">
        <input 
          type="search" 
          id="${prefix}-area-search-input" 
          class="form-input area-search-input" 
      ...
      <div class="area-picker-box" id="${prefix}-area-picker-box" style="max-height:160px;overflow-y:auto;display:flex;flex-wrap:wrap;gap:6px;padding:6px;border-radius:var(--radius-md);background:var(--surface);border:1px solid var(--border)">
      ...
      <div class="area-no-match" id="${prefix}-area-no-match" style="display:none;padding:8px;font-size:12px;color:var(--text-muted);text-align:center">
        لم نجد قرية مطابقة. يمكنك اختيار <strong style="color:var(--secondary,#F5A623);cursor:pointer" class="area-other-btn">✏️ بلد أو قرية أخرى...</strong> وكتابتها يدوياً.
      </div>

      <!-- Hidden Input for Form Submission -->
      <input type="hidden" id="${prefix}-area" class="b-area area-hidden-input" value="${escAttr(isCustomArea ? currentArea : currentAreaVal)}" />

      <!-- Custom Area Text Input (Shows when 'other' is selected) -->
      <div class="form-group animate-fade-in custom-area-group" id="${prefix}-custom-area-group" style="margin-top:10px;${isCustomArea ? '' : 'display:none'}">
        ...
        <input type="text" id="${prefix}-custom-area" class="form-input custom-area-input" placeholder="مثال: ميت مرجا سلسيل، الكردي، أو أي قرية أخرى..." value="${escAttr(isCustomArea ? currentArea : '')}" />
      </div>
    </div>
  `;
}

function initAreaSelectEvents(rootEl) {
  if (!rootEl) return;
  const containers = rootEl.classList?.contains('area-picker-container')
    ? [rootEl]
    : Array.from(rootEl.querySelectorAll('.area-picker-container'));

  containers.forEach(container => {
    if (container.__areaPickerInitialized) return;
    container.__areaPickerInitialized = true;

    const searchInput = container.querySelector('.area-search-input');
    const pickerBox = container.querySelector('.area-picker-box');
    const pills = pickerBox ? pickerBox.querySelectorAll('.area-select-pill') : [];
    const noMatch = container.querySelector('.area-no-match');
    const hiddenInput = container.querySelector('.area-hidden-input');
    const customGroup = container.querySelector('.custom-area-group');
    const customInput = container.querySelector('.custom-area-input');
    const badgeName = container.querySelector('.area-selected-name');
    const otherBtn = container.querySelector('.area-other-btn');
    ...
    otherBtn?.addEventListener('click', () => {
      container.querySelector('.area-select-pill[data-area-name="other"]')?.click();
    });
      ...
      pill.addEventListener('click', () => {
        ...
        if (areaName === 'other') {
          ...
          if (customInput) {
            ...
            if (hiddenInput) hiddenInput.value = val || 'المنزلة';
            if (badgeName) badgeName.textContent = val || 'بلد مخصص';
          }
        } else {
          if (customGroup) customGroup.style.display = 'none';
          if (hiddenInput) hiddenInput.value = areaName;
          if (badgeName) badgeName.textContent = areaName;
        }
...
function openBranchModal({ place, branchIndex = -1, user, onSave = null }) {
  ...
  const modalHtml = `
    <form id="branch-quick-modal-form" style="display:flex;flex-direction:column;gap:14px;padding:6px 0">
      ...
      <!-- Area Selection for Branch Modal -->
      ${renderAreaSelectForm({
        prefix: 'bm',
        initialArea: branch.area || place?.area || 'المنزلة',
        label: 'المنطقة أو المدينة لهذا الفرع (المنزلة أو المطرية أو القرى)',
        isRequired: true
      })}
    ...
    buttons: [
      {
        ...
        onClick: async (e, btn) => {
          ...
          let area = document.getElementById('bm-area')?.value.trim() || '';
          if (area === 'other') {
            area = document.getElementById('bm-custom-area')?.value.trim() || 'المنزلة';
          }
          if (!area) area = 'المنزلة';
  ...
  const modalForm = document.getElementById('branch-quick-modal-form');
  if (modalForm) initAreaSelectEvents(modalForm);
...
async function renderPlaceFormSection($container, user, placeId = null) {
  ...
  function addBranchRow(initialData = null) {
    ...
    row.innerHTML = `
      ...
      <!-- Area Selection for this branch -->
      ${renderAreaSelectForm({
        prefix: `b-area-${idx}-${Date.now().toString(36)}`,
        initialArea: initialData?.area || place?.area || 'المنزلة',
        label: 'المنطقة أو القرية لهذا الفرع (المنزلة أو المطرية أو القرى)',
        isRequired: true
      })}
      <div class="form-group">
  ...
  // Live Area Search Filter & Pill Selection (55 villages + search + custom)
  initAreaSelectEvents(document.getElementById('p-location-section'));
