...
function _footerHTML() {
  ...
  const groups = isEn ? [
    {title:'Quick Links',links:[['/en/','Home'],['/en/places/','Places Directory'],['/en/categories/','Categories'],['/en/offers/','Offers'],['/en/blog/','Blog'],['/en/favorites/','Favorites'],['/en/about.html','About Us']]},
    ...
    {title:'روابط سريعة',links:[['/index.html','الصفحة الرئيسية'],['/places.html','دليل الأماكن'],['/categories.html','التصنيفات'],['/offers.html','العروض'],['/blog','المدونة'],['/favorites.html','المفضلة'],['/about.html','عن الدليل']]},
...
function _loadShellCSS(){
  if(!document.getElementById('shell-mobile-fixes-css')){
    ...
    l.rel='stylesheet';
    l.href='/src/css/mobile-shell-fixes.css?v=a58f9ed6';
...
export function installDisplayControlsRail(){
  ...
  function scheduleCollapse(delay=1800){clearTimeout(collapseTimer);collapseTimer=setTimeout(()=>{rail.classList.remove('is-expanded');rail.classList.add('is-collapsed')},delay)}
  function revealRail(){clearTimeout(collapseTimer);rail.classList.remove('is-collapsed');rail.classList.add('is-expanded')}
  ...
  window.addEventListener('scroll',()=>{if(rail.classList.contains('is-expanded'))scheduleCollapse(400)},{passive:true});
  document.addEventListener('pointerdown',e=>{if(rail.classList.contains('is-expanded')&&!rail.contains(e.target)&&!edgeTrigger.contains(e.target))scheduleCollapse(100)},{passive:true});
}
