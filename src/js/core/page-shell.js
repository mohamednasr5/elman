...
function _bottomNavHTML(active = '') {
  ...
  return `<nav class="bottom-nav" id="bottom-nav" role="navigation" aria-label="${isEn ? 'Quick Navigation' : 'تنقل سريع'}">
    ...
    <button type="button" class="bottom-nav__item" id="bottom-nav-more-btn" aria-expanded="false" aria-label="${items[3][2]}"><span class="bottom-nav__icon">${items[3][1]}</span><span class="bottom-nav__label">${items[3][2]}</span></button>
  </nav>`;
}
