...
export function installI18nHardening(){
  ...
  if(!document.getElementById(cssId)){const link=document.createElement('link');link.id=cssId;link.rel='stylesheet';link.href='/src/css/i18n-layout.css?v=20260913';document.head.appendChild(link)}
  const scan=()=>document.querySelectorAll('[data-i18n],[data-i18n-aria],[data-i18n-title],button,a,label,span,p,h1,h2,h3,h4,h5,h6,small,strong,input,textarea').forEach(el=>{if(el.hasAttribute('data-i18n')){const value=t(el.getAttribute('data-i18n'));if(el.matches('input,textarea'))el.setAttribute('placeholder',value);else el.textContent=value}if(el.hasAttribute('data-i18n-aria'))el.setAttribute('aria-label',t(el.getAttribute('data-i18n-aria')));if(el.hasAttribute('data-i18n-title'))el.setAttribute('title',t(el.getAttribute('data-i18n-title')));translateLegacyElement(el)});
  scan();
  const observer=new MutationObserver(ms=>{for(const m of ms)for(const n of m.addedNodes)if(n.nodeType===1){translateLegacyElement(n);n.querySelectorAll?.('[data-i18n],[data-i18n-aria],[data-i18n-title],button,a,label,span,p,h1,h2,h3,h4,h5,h6,small,strong,input,textarea').forEach(translateLegacyElement)}});
