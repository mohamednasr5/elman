...
async function d1Fetch(path, options = {}) {
  ...
  if (!res.ok) {
    throw new Error(data?.error || data?.message || `Worker HTTP ${res.status}`);
  }
  return data;
}
...
async function d1GetBusiness(path) {
  ...
  if (root === 'categories') {
    ...
    const id = parts[1];
    return list.find(c => String(c.id || c.slug) === String(id)) || null;
  }
  ...
  if (root === 'places') {
    ...
    if (parts.length >= 3 && parts[2] === 'reviews') {
      ...
      const data = await d1Fetch(`/api/reviews?place_id=${encodeURIComponent(placeId)}`);
      const list = Array.isArray(data.data) ? data.data : [];
      if (reviewId) return list.find(r => String(r.id) === String(reviewId)) || null;
    ...
    const id = parts[1];
    const data = await d1Fetch(`/api/places?id=${encodeURIComponent(id)}`);
...
async function d1WriteBusiness(path, method, data = null) {
  ...
  if (root === 'places') {
    if (parts.length >= 3 && parts[2] === 'reviews') {
      ...
      const reviewId = parts[3];
      if (method === 'DELETE') {
        if (reviewId) return d1Fetch(`/api/reviews?id=${encodeURIComponent(reviewId)}`, { method: 'DELETE' });
        return d1Fetch(`/api/reviews?place_id=${encodeURIComponent(placeId)}`, { method: 'DELETE' });
      }
      if (method === 'POST') {
        return d1Fetch('/api/reviews', { method: 'POST', body: JSON.stringify({ ...(data || {}), place_id: data?.place_id || placeId }) });
      }
      if (method === 'PUT') {
        if (!reviewId) throw new Error('Review ID is required for PUT');
        return d1Fetch(`/api/reviews?id=${encodeURIComponent(reviewId)}`, { method: 'PUT', body: JSON.stringify(data || {}) });
      }
    }
    ...
    if (parts.length === 2 && method === 'DELETE') {
      return d1Fetch(`/api/places/${encodeURIComponent(parts[1])}`, { method: 'DELETE' });
    }
...
export async function getPlaceBySlug(slug) {
  ...
  try {
    ...
    if (workerRes.ok) {
      ...
      if (data && data.success && data.data) {
        ...
        const rawPlace = Array.isArray(data.data)
          ? data.data.find(item => item && (item.slug === cleanSlug || item.id === cleanSlug || (item.slug && item.slug.toLowerCase() === lower)))
          : data.data;
  ...
  try {
    ...
    if (localPlaces && localPlaces.length) {
      const lower = cleanSlug.toLowerCase();
      const p = localPlaces.find(item => item && (
        item.slug === cleanSlug || item.id === cleanSlug || item._key === cleanSlug ||
        (item.slug && item.slug.toLowerCase() === lower)
      ));
      if (p) return p;
    }
  } catch (_) {}
...
export async function saveCategoryD1(category) {
  if (!category || (!category.name && !category.slug && !category.id)) {
    throw new Error('بيانات التصنيف غير مكتملة');
  }

  const rawSlug = category.slug || category.id || category._key;
  const slug = String(rawSlug).trim().toLowerCase().replace(/\s+/g, '-');
  const payload = {
    id: category.id || slug,
    _key: category.id || slug,
    slug,
    name: category.name,
    nameEn: category.nameEn || category.name_en || slug,
    icon: category.icon || '📁',
    description: category.description || '',
    color: category.color || '#1B4F72',
    order: Number(category.order != null ? category.order : category.sort_order) || Date.now(),
    placeCount: Number(category.placeCount || category.place_count) || 0
  };

  // 1. Immediately store in IndexedDB and clear in-memory cache for 0ms reactivity
  try {
  ...
  try {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok && data.error && !data.error.includes('exceeded D1')) {
      console.warn('[saveCategoryD1] Worker error:', data.error);
    }
  } catch (err) {
    console.warn('[saveCategoryD1] Network/D1 sync handled:', err.message);
  }

  return payload;
}

/** Delete Category from Cloudflare D1 + Local IndexedDB */
export async function deleteCategoryD1(catId) {
  if (!catId) return;
  const cleanId = String(catId).trim();

  // 1. Remove from local IndexedDB & memory cache
  try {
    await idbDelete(STORES.CATEGORIES, cleanId);
    clearDbCache('categories');
  } catch (_) {}
  ...
  try {
    ...
    console.warn('[deleteCategoryD1] D1 deletion handled:', err.message);
  }
}

/** Get all categories (ordered) - Cached in IndexedDB & Memory (Cloudflare D1 Primary) */
export async function getCategories() {
  ...
  try {
    ...
    if (workerRes.ok) {
      ...
      if (data && data.success && Array.isArray(data.data) && data.data.length > 0) {
        const categories = data.data.map(c => ({
          ...
          order: Number(c.order != null ? c.order : c.sort_order) || 0,
    ...
    console.debug('[getCategories] D1 fetch error, trying IndexedDB:', workerErr.message);
  }

  // 2. Fallback to local IndexedDB (0ms offline/cached)
  try {
    ...
    if (idbCats && idbCats.length > 0) {
      ...
      return setCache(cacheKey, idbCats);
    }
  } catch (_) {}
  ...
  try {
    ...
    (places || []).forEach(p => {
      if (p && Array.isArray(p.offers)) {
        p.offers.forEach(o => {
          if (o && (!o.endDate || o.endDate > now)) {
            offers.push({ ...o, placeId: p.id, placeName: p.name, placeSlug: p.slug });
          }
        });
      }
    });
...
export async function getPlaceOffers(placeId) {
  ...
  try {
    ...
    if (place && Array.isArray(place.offers)) {
      const now = Date.now();
      return place.offers.filter(o => !o.endDate || o.endDate > now);
    }
    return [];
  } catch (err) {
...
export async function getPlaceProducts(placeId, { limit = 50, includePending = false } = {}) {
  ...
  try {
    const place = await getPlace(placeId);
    if (place && Array.isArray(place.products)) {
      const prods = place.products.filter(prod => {
...
export async function getAllProducts() {
  try {
    ...
    (places || []).forEach(place => {
      if (place && Array.isArray(place.products)) {
        place.products.forEach(prod => {
          if (!prod) return;
          all.push({
            id: prod.id || prod._key,
            _key: prod.id || prod._key,
            placeId: place.id,
            placeName: place.name || 'مكان غير معروف',
            placeSlug: place.slug || place.id,
            ...prod
          });
        });
      }
...
export async function adminApproveProduct(placeId, productId) {
  ...
  const place = await getPlace(placeId);
  if (place && Array.isArray(place.products)) {
    const prod = place.products.find(p => (p.id || p._key) === productId);
    if (prod) {
      prod.status = 'approved';
      prod.isApproved = true;
      prod.approvedAt = Date.now();
      await syncPlaceToWorkerD1(placeId, place);
      await idbPut(STORES.PLACES, place);
    }
  }
...
export async function adminRejectProduct(placeId, productId, reason = '') {
  ...
  return { status: 'rejected', isApproved: false };
}
...
export async function adminDeleteProduct(placeId, productId) {
  ...
  const place = await getPlace(placeId);
  if (place && Array.isArray(place.products)) {
    place.products = place.products.filter(p => (p.id || p._key) !== productId);
    place.productCount = Math.max(0, (place.productCount || 1) - 1);
    await syncPlaceToWorkerD1(placeId, place);
    await idbPut(STORES.PLACES, place);
  }
}
...
export async function getSettings() {
  const cached = getCached('site_settings', 600000);
  if (cached) return cached;
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data && data.success && data.data && typeof data.data === 'object' && Object.keys(data.data).length > 0) {
        return setCache('site_settings', data.data);
      }
    }
  } catch (_) {}
  ...
  // Safe defaults
  const defaults = {
    siteName: 'دليل المنزلة والمطرية الرقمي',
    contact: { whatsapp: '01000000000' }
  };
  return setCache('site_settings', defaults);
}

/** Save site settings to Cloudflare Worker */
