...
async function tursoFetch(path, options = {}) {
  ...
  if (path === '/api/categories' && typeof window !== 'undefined' && window.__CATEGORIES_PREFETCH__ && !options.method) {
    try {
      const prefetched = await window.__CATEGORIES_PREFETCH__;
      if (prefetched && prefetched.success !== false) return prefetched;
    } catch (_) {}
  }
...
export async function updateUserTurso(uid, { role, status, name, email, phone, points, note } = {}) {
  const body = {};
  for (const [key, value] of Object.entries({ role, status, name, email, phone, points, note })) {
...
async function _syncPublishedPlaces(limit = 100, cacheKey = '') {
  try {
    let data = null;
    if (typeof window !== 'undefined' && window.__PLACES_PREFETCH__ && limit === 100) {
      try { data = await window.__PLACES_PREFETCH__; } catch (_) {}
    }
    if (!data || !data.success) {
      const workerRes = await fetch(`${WORKER_URL}/api/places?limit=${limit}`, {
        signal: AbortSignal.timeout(6000)
      });
      if (workerRes.ok) data = await workerRes.json();
    }
    if (data && data.success && Array.isArray(data.data)) {
        const places = [];
        const allForIdb = [];
...
export async function broadcastNewPlaceNotification(place) {
  ...
  const targetUrl = '/place/' + encodeURIComponent(place.slug || place._key || placeId) + '/';
  ...
  try {
    ...
    if (ownerId && ownerId !== userId) {
      ...
      const ownerNotification = {
        ...
        actionUrl: `/place/${encodeURIComponent(placeSlug || place?.slug || placeId)}/#reviews`,
        createdAt: Date.now(),
        isRead: false
      };
