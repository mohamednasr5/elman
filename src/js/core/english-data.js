import { VILLAGE_NAMES_EN, CATEGORY_NAMES_EN, translateCategory } from '../utils/category-i18n.js';
const ARABIC_RE = /[\u0600-\u06FF]/;
...
export function englishArea(place, fallback = 'Local Area') { const raw = place?.areaEn ?? place?.area_en ?? place?.area ?? ''; if (VILLAGE_NAMES_EN[raw]) return VILLAGE_NAMES_EN[raw]; return englishText(raw, fallback); }
export function englishCategory(place, fallback = 'Local Services') { const raw = place?.categoryNameEn ?? place?.category_en ?? place?.categoryName ?? place?.customCategory ?? place?.categoryId ?? ''; const translated = translateCategory(raw, true); if (translated && !hasArabic(translated)) return translated; const key = String(raw).trim().toLowerCase().replace(/_/g, '-'); return CATEGORY_NAMES_EN[key] || englishText(raw, fallback); }
export function projectPlaceToEnglish(place = {}) { return { ...place, _en: true, displayName: englishName(place), displayDescription: englishDescription(place), displayAddress: englishAddress(place), displayArea: englishArea(place), displayCategory: englishCategory(place) }; }
