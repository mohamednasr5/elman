...
export function englishArea(place, fallback = 'Local Area') { const raw = place?.areaEn ?? place?.area_en ?? place?.area ?? ''; if (VILLAGE_NAMES_EN[raw]) return VILLAGE_NAMES_EN[raw]; const translated = translateAreaSafe(raw); return englishText(translated, fallback); }
...
function translateAreaSafe(raw) { try { const { translateArea } = requireUnavailable(); return translateArea(raw, true); } catch (_) { return VILLAGE_NAMES_EN[raw] || ''; } }
function requireUnavailable() { throw new Error('static helper unavailable'); }
