...
export async function createPlace(placeData, currentUser) {
  ...
  // Save to IndexedDB local cache immediately for 0ms reactivity
  await idbPut(STORES.PLACES, { id: placeId, ...newPlace }).catch(() => {});

  // Update category placeCount locally
...
export async function deletePlace(placeId, ownerId) {
  const place = (await getPlace(placeId)) || (await idbGet(STORES.PLACES, placeId));
  if (!place) return;
  ...
  if (place.categoryId) {
    try {
      const cat = await idbGet(STORES.CATEGORIES, place.categoryId);
      if (cat && cat.placeCount > 0) {
        cat.placeCount -= 1;
        await idbPut(STORES.CATEGORIES, cat);
      }
    } catch (_) {}
  ...
  try {
    await fetch(`${WORKER_URL}/api/verification-requests`, {
      ...
      body: JSON.stringify({
        ...
        owner_id: user.uid,
        owner_name: user.name || user.displayName || '',
