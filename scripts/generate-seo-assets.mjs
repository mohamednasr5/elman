...
const ROOT=process.cwd(),SITE='https://dalilmanzala.com',API=`${SITE}/api/places`,ARTICLE_API=`${SITE}/api/articles`,TODAY=new Date().toISOString().slice(0,10);
...
const image=p=>{const candidates=[p.coverImageUrl,p.cover_image_url,p.coverUrl,p.cover_url,p.imageUrl,p.image_url,p.logoUrl,p.logo_url];let gallery=p.gallery||p.imageUrls||p.image_urls||p.photos||[];if(typeof gallery==='string'){try{gallery=JSON.parse(gallery)}catch(_){gallery=[]}};candidates.push(...(Array.isArray(gallery)?gallery:[]));const v=candidates.find(x=>typeof x==='string'&&x.trim());return v?abs(v):''};
...
async function main(){const places=await fetchAllPlaces();const articles=await fetchAllArticles();if(!places.length)throw new Error('No indexable places; refusing to replace SEO assets.');const ar=[],en=[],cats=new Set();for(const [a,b,f,file] of STATIC){const lm=gitDate(file);ar.push(entry(a,lm,f,a==='/'?'1.0':'0.7',{ar:a,en:b}));en.push(entry(b,lm,f,b==='/en/'?'1.0':'0.7',{ar:a,en:b}))}for(const p of places){const c=p.customCategory||p.category||p.categoryId||p.category_id;if(c)cats.add(String(c).trim().toLowerCase().replace(/\s+/g,'-'))}const ca=[],ce=[];for(const c of [...cats].sort()){const a=`/category/${encodeURIComponent(c)}/`,b=`/en/category/${encodeURIComponent(c)}/`;ca.push(entry(a,null,'weekly','0.8',{ar:a,en:b}));ce.push(entry(b,null,'weekly','0.8',{ar:a,en:b}))}const pa=[],pe=[],urls=[];for(const p of places){const a=arPlaceUrl(p),b=enPlaceUrl(p),alt={ar:a,en:b,image:image(p)},lm=lastmod(p);pa.push(entry(a,lm,'weekly','1.0',alt));pe.push(entry(b,lm,'weekly','1.0',alt));urls.push(abs(a),abs(b))}
const articleEntries=[];
for(const article of articles){
  const s=String(article?.slug||article?.id||'').trim(); if(!s) continue;
  const loc=`/article/${pathSlug(s)}/`;
  const lm=lastmod(article);
  const img=article?.coverImageUrl||article?.cover_image_url||article?.coverUrl||article?.cover_url||'';
...
fs.writeFileSync(path.join(ROOT,'.indexnow-urls.json'),JSON.stringify({urls:[...new Set(urls)]},null,2)+'\n');console.log(`SEO assets: ${places.length} places, ${articles.length} articles, ${cats.size} categories, ${STATIC.length} static pairs.`)}
