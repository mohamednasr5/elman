...
const required=[
  ...
  'assets/images/default-cover.jpg','assets/images/default-logo.jpg'
];
const missing=required.filter(p=>!fs.existsSync(path.join(root,p)));
if(missing.length){console.error('Missing English frontend/default asset files:',missing.join(', '));process.exit(1);}
...
const assets=fs.readFileSync(path.join(root,'src/js/utils/category-assets.js'),'utf8');
if(!assets.includes("DEFAULT_PLACE_COVER = '/assets/images/default-cover.jpg'")) throw new Error('Default cover must point to the existing JPG asset');
if(!assets.includes("DEFAULT_PLACE_LOGO = '/assets/images/default-logo.jpg'")) throw new Error('Default logo must point to the existing JPG asset');
const redirects=fs.readFileSync(path.join(root,'_redirects'),'utf8');
for(const rule of ['/assets/images/default-cover.png  /assets/images/default-cover.jpg  301','/assets/images/default-logo.png   /assets/images/default-logo.jpg   301']) if(!redirects.includes(rule)) throw new Error(`Missing compatibility redirect: ${rule}`);
console.log(`English frontend structural validation passed (${enPages.length} English page modules). Default cover/logo assets validated.`);