...
دليل المنزلة والمطرية الرقمي (Dalil Manzala & Matariya) هو دليل محلي مفتوح لاكتشاف الأنشطة والخدمات والأماكن في المنطقة. حالة التوثيق ودقة أي معلومة تعتمد على البيانات الحالية المسجلة لكل مكان.
...
- النطاق الجغرافي المشمول: المنزلة والمطرية والمجتمعات والقرى التي توجد بها أماكن منشورة في الدليل؛ لا تُنسب إحداثيات عامة للأنشطة الفردية ما لم تكن مسجلة فعلياً.
...
## Blog & Articles
- [المدونة المحلية](https://dalilmanzala.com/blog/): مقالات منشورة مرتبطة بالأماكن والأنشطة.
- [مقال](https://dalilmanzala.com/article/{slug}/): كل مقال منشور له رابط ثابت ويمكن الرجوع منه مباشرة إلى صفحة المكان المرتبط.
...
// ── Turso: Sync/Update Place (POST/PUT /api/places/sync or /api/places) ──
  if ((url.pathname === '/api/places/sync' || url.pathname === '/api/places') && (request.method === 'POST' || request.method === 'PUT')) {
    try {
      ...
      if (indexNowUrls.length && ctx && typeof ctx.waitUntil === 'function') {
        ctx.waitUntil(notifyIndexNow(indexNowUrls, env).catch(err => {
          console.warn('[IndexNow background error]:', err?.message || err);
        }));
      }
